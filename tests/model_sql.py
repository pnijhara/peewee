...
class TestModelSQL(ModelDatabaseTestCase):
    ...
    def test_select(self):
        query = (
            Person.select(Person.first, Person.last, fn.COUNT(Note.id).alias("ct"))
            .join(Note)
    ...
    def test_where_coerce(self):
        ...
        self.assertSQL(
            query,
            ('SELECT "t1"."last" FROM "person" AS "t1" ' 'WHERE ("t1"."id" = ?)'),
            [1337],
        )
        ...
        self.assertSQL(
            query,
            ('SELECT "t1"."last" FROM "person" AS "t1" ' 'WHERE ("t1"."first" = ?)'),
            ["foo"],
        )

    def test_group_by(self):
        query = (
            User.select(User, fn.COUNT(Tweet.id).alias("tweet_count"))
    ...
    def test_group_by_extend(self):
        query = (
            User.select(User, fn.COUNT(Tweet.id).alias("tweet_count"))
            .join(Tweet, JOIN.LEFT_OUTER)
            .group_by_extend(User.id)
            .group_by_extend(User.username)
        )
        self.assertSQL(
            query,
            (
    ...
    def test_order_by_extend(self):
        query = (
            User.select().order_by_extend(User.username.desc()).order_by_extend(User.id)
        )
        self.assertSQL(
            query,
            (
    ...
    def test_model_alias(self):
        TA = Tweet.alias()
        query = (
            User.select(User, fn.COUNT(TA.id).alias("tc"))
    ...
    def test_model_alias_with_schema(self):
        ...
        self.assertSQL(
            query,
            ('SELECT "t1"."id", "t1"."content" ' 'FROM "notes"."note" AS "t1"'),
            [],
        )
    ...
    def test_filter_expressions(self):
        query = User.filter(
            DQ(username__in=["huey", "zaizee"]) | (DQ(id__gt=2) & DQ(id__lt=4))
        )
        self.assertSQL(
            query,
            (
    ...
    def test_insert(self):
        ...
        self.assertSQL(
            ...
            ["huey", "cat", datetime.date(2011, 1, 1)],
        )
    ...
    def test_replace(self):
        ...
        self.assertSQL(
            query,
            ('INSERT OR REPLACE INTO "person" ("first", "last") ' "VALUES (?, ?)"),
            ["huey", "cat"],
        )

    def test_insert_many(self):
        query = Note.insert_many(
            (
    ...
    def test_insert_many_defaults(self):
        ...
        s3 = {"counter": 3}
        self.assertSQL(
            Sample.insert_many([s2, s3]),
            ('INSERT INTO "sample" ("counter", "value") VALUES (?, ?), (?, ?)'),
            [2, 2.0, 3, 1.0],
        )

        self.assertSQL(
            Sample.insert_many([s3, s2]),
            ('INSERT INTO "sample" ("counter", "value") VALUES (?, ?), (?, ?)'),
            [3, 1.0, 2, 2.0],
        )
    ...
    def test_insert_returning(self):
        ...
        self.assertSQL(
            query,
            ('INSERT INTO "user" ("username") ' 'VALUES (?) RETURNING "user"."id"'),
            ["zaizee"],
        )
        ...
        query = Person.insert({Person.name: "foo"}).returning(
            Person.ssn.alias("new_ssn")
        )
        self.assertSQL(
            query,
            (
                'INSERT INTO "person" ("name") VALUES (?) '
                'RETURNING "person"."ssn" AS "new_ssn"'
            ),
            ["foo"],
        )
    ...
    def test_insert_get_field_values(self):
        ...
        for query in queries:
            self.assertSQL(
                query, ('INSERT INTO "user" ("username") ' "VALUES (?)"), ["a"]
            )
    ...
    def test_insert_modelalias(self):
        UA = User.alias("ua")
        self.assertSQL(
            UA.insert({UA.username: "huey"}),
            ('INSERT INTO "users" ("username") VALUES (?)'),
            ["huey"],
        )
        self.assertSQL(
            UA.insert(username="huey"),
            ('INSERT INTO "users" ("username") VALUES (?)'),
            ["huey"],
        )
    ...
    def test_update(self):
        ...
        query = Stat.update(
            {Stat.count: Stat.count + 1, Stat.timestamp: datetime.datetime(2017, 1, 1)}
    ...
    def test_update_from(self):
        ...
        query = (
            Account.update(
                contact_first=SalesPerson.first, contact_last=SalesPerson.last
            )
            .from_(SalesPerson)
        ...
        self.assertSQL(
            ...
            (
                ...
                'FROM "tweet" AS "t1" WHERE ("t1"."content" = ?)'
            ),
            ["tx"],
        )
    ...
    def test_delete(self):
        query = Note.delete().where(
            Note.author << (Person.select(Person.id).where(Person.last == "cat"))
        )
        self.assertSQL(
            query,
            (
        ...
        self.assertSQL(
            query, ('DELETE FROM "note" WHERE ("note"."author_id" = ?)'), [123]
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                (
                    ...
                    'WHERE ("relationship"."from_user_id" = ?)',
                    [1],
                ),
                (
                    'DELETE FROM "relationship" '
                    'WHERE ("relationship"."to_user_id" = ?)',
                    [1],
                ),
    ...
    def test_aliases(self):
        ...
        query = (
            D.select(D.d, C.c)
            .join(C)
            .where(C.b_link << (B.select(B.id).join(A).where(A.a == "a")))
        )
        self.assertSQL(
            query,
            (
...
class TestOnConflictSQL(ModelDatabaseTestCase):
    ...
    def test_conflict_target_constraint_where(self):
        ...
        query = UKVP.insert_many(data, fields).on_conflict(
    ...
    def test_union_from(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'WHERE ("t3"."gamma" < ?)) AS "cq"'
            ),
            [2, 3, 0],
        )

    def test_parentheses(self):
        query = Alpha.select().where(Alpha.alpha < 2) | Beta.select(
            Beta.id, Beta.beta
        ).where(Beta.beta > 3)
        self.assertSQL(
            query,
...
class TestModelIndex(BaseTestCase):
    ...
    def test_model_index(self):
        ...
        aidx = ModelIndex(
            Article, (Article.timestamp.desc(), Article.flags.bin_and(4)), unique=True
        )
        self.assertSQL(
            aidx,
            (
                'CREATE UNIQUE INDEX IF NOT EXISTS "article_timestamp" '
                'ON "article" ("timestamp" DESC, ("flags" & ?))'
            ),
            [4],
        )
...
class TestModelArgument(BaseTestCase):
    ...
    def test_model_as_argument(self):
        ...
        query = Post.select(Post.id, fn.score(Post).alias("score")).order_by(
            Post.timestamp
        )
        self.assertSQL(
            query,
