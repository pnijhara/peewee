...
class BaseTestQueueDatabase(object):
    ...
    def test_multiple_threads(self):
        ...
        self.database.start()
        threads = [
            self.create_thread(create_rows, i, self.n_rows)
    ...
    def test_waiting(self):
        ...
        threads = [
            self.create_thread(create_user, name)
    ...
    def test_next_method(self):
        ...
        self.assertEqual(
            next(self.database.execute_sql("PRAGMA journal_mode"))[0], "wal"
        )
...
class TestThreadedDatabaseThreads(BaseTestQueueDatabase, BaseTestCase):
    ...
    def test_timeout(self):
        ...
        def do_query():
            cursor = self.database.execute_sql("select slow(?)", (0.01,), commit=True)
