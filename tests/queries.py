...
class TestQueryExecution(DatabaseTestCase):
    ...
    def create_user_tweets(self, username, *tweets):
        ...
        for tweet in tweets:
            Tweet.insert({Tweet.user_id: user_id, Tweet.content: tweet}).execute()
        return user_id
    ...
    def test_select_peek_first(self):
        ...
        self.assertEqual(query.peek(n=2), [{"content": "meow"}, {"content": "purr"}])
    ...
    def test_select_get(self):
        ...
        self.assertEqual(
            User.select().where(User.username == "huey").get(),
    ...
    def test_scalar(self):
        ...
        (Register.insert([{Register.value: value} for value in values]).execute())
    ...
    def test_slicing_select(self):
        values = [1.0, 1.0, 2.0, 3.0, 5.0, 8.0]
        (Register.insert([(v,) for v in values], columns=(Register.value,)).execute())

        query = Register.select(Register.value).order_by(Register.value).tuples()
        with self.assertQueryCount(1):
            self.assertEqual(query[0], (1.0,))
            self.assertEqual(query[:2], [(1.0,), (1.0,)])
            self.assertEqual(query[1:4], [(1.0,), (2.0,), (3.0,)])
            self.assertEqual(query[-1], (8.0,))
            self.assertEqual(query[-2], (5.0,))
            self.assertEqual(query[-2:], [(5.0,), (8.0,)])
            self.assertEqual(query[2:-2], [(2.0,), (3.0,)])
...
class TestQueryCloning(BaseTestCase):
    ...
    def _do_test_clone(self, User, Tweet):
        ...
        self.assertSQL(qw, base_sql + ' WHERE ("t1"."id" > ?)', [3])
        self.assertSQL(
            qw2, base_sql + (' WHERE (("t1"."id" > ?) ' 'AND ("t1"."id" < ?))'), [3, 6]
        )
        ...
        self.assertSQL(query, base_sql, [])
        self.assertSQL(qo, base_sql + ' ORDER BY "t1"."id"', [])
        self.assertSQL(qo2, base_sql + ' ORDER BY "t1"."content", "t1"."id"', [])
