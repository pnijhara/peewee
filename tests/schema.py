...
class TMIndexes(TestModel):
    ...
    class Meta:
        indexes = ((("alpha", "beta"), True), (("beta", "gamma"), False))
...
idx = Article.index(Article.name, Article.timestamp, Article.flags.bin_and(4)).where(
    Article.status == 1
)
...
class TestModelDDL(ModelDatabaseTestCase):
    ...
    def test_model_indexes_complex_columns(self):
        ...
        index = Taxonomy.index(name, Taxonomy.name_class).where(
            Taxonomy.name_class == "scientific name"
        )
    ...
    def test_without_pk(self):
        ...
        self.assertCreateTable(NoPK, [('CREATE TABLE "no_pk" ("data" TEXT NOT NULL)')])
    ...
    def test_db_table(self):
        ...
        self.assertCreateTable(
            A, ['CREATE TABLE "A_tbl" ("id" INTEGER NOT NULL PRIMARY KEY)']
        )
        self.assertCreateTable(
            B,
            [
    ...
    def test_table_options(self):
        class TOpts(TestModel):
            ...
            class Meta:
                database = self.database
                options = {"CHECKSUM": 1, "COMPRESSION": "lz4"}
    ...
    def test_fk_non_pk_ddl(self):
        class A(Model):
            ...
            df = DecimalField(
                max_digits=4, decimal_places=2, auto_round=True, unique=True
            )
    ...
    def test_deferred_foreign_key(self):
        ...
        self.assertEqual(Language._meta.fields["selected_snippet"].rel_model, Snippet)
        ...
        sql, params = Language._schema._create_foreign_key(
            Language.selected_snippet
        ).query()
        self.assertEqual(
            sql,
            (
        ...
        sql, params = SnippetComment._schema._create_foreign_key(
    ...
    def test_deferred_foreign_key_inheritance(self):
        ...
        class TimestampTweet(Tweet, WithTimestamp):
            pass
...
class TestCreateAs(ModelTestCase):
    ...
    def test_create_as(self):
        status = Case(NoteX.status, ((1, "published"), (2, "draft"), (9, "deleted")))

        query = NoteX.select(
            NoteX.id, NoteX.content, NoteX.timestamp, status.alias("status")
