...
class TestDataSet(ModelTestCase):
    ...
    def test_item_apis(self):
        ...
        del users[2]
        self.assertEqual(
            list(users), [{"id": 1, "username": "charlie", "color": None, "kind": None}]
        )
    ...
    def test_special_char_table(self):
        ...
        self.database.execute_sql('INSERT INTO "hello!!world" VALUES (?)', ("test",))
    ...
    def test_column_preservation(self):
        ...
        self.assertEqual(sorted(data), [("", "BOOK2"), ("BOOK1", "")])
    ...
    def test_case_insensitive(self):
        ...
        tables = sorted(self.dataset.tables)
        self.assertEqual(
            tables, ["HueyMickey", "SomeTable", "category", "note", "user"]
        )
    ...
    def test_update_cache(self):
        self.assertEqual(sorted(self.dataset.tables), ["category", "note", "user"])
    ...
    def assertQuery(self, query, expected, sort_key="id"):
        ...
        self.assertEqual(sorted(list(query), key=key), sorted(expected, key=key))

    def test_insert(self):
        self.create_users()
        user = self.dataset["user"]

        expected = [{"username": "charlie"}, {"username": "huey"}]
        ...
        self.assertEqual(
            user.find_one(username="mickey"), {"username": "mickey", "age": 5}
        )
    ...
    def test_update(self):
        ...
        res = user.update(favorite_color="blue", username="huey", columns=["username"])
    ...
    def test_find(self):
        ...
        def assertUsernames(query, expected):
            self.assertEqual(sorted(row["username"] for row in query), sorted(expected))
        ...
        for username in ["zaizee", "huey"]:
            user.update(favorite_color="blue", username=username, columns=["username"])

        assertUsernames(
            user.find(favorite_color="green"), ["charlie", "mickey", "peewee"]
        )
        assertUsernames(user.find(favorite_color="blue"), ["zaizee", "huey"])
        assertUsernames(
            user.find(favorite_color="green", username="peewee"), ["peewee"]
        )
    ...
    def test_export(self):
        ...
        self.assertEqual(
            buf.getvalue(), ('[{"username": "charlie"}, {"username": "huey"}]')
        )

        buf = StringIO()
        self.dataset.freeze(user.all(), "csv", file_obj=buf)
        self.assertEqual(buf.getvalue().splitlines(), ["username", "charlie", "huey"])
    ...
    def test_freeze_thaw(self):
        ...
        self.assertEqual(
            list(note.all()),
            [{"id": 1, "user_id": "charlie", "content": "foo", "timestamp": note_ts}],
        )
    ...
    def test_import_json(self):
        ...
        self.assertEqual(set(names), set(["charlie", "huey", "mickey", "zaizee"]))
        ...
        self.assertEqual(set(table.columns), set(["id", "name", "bar", "foo"]))
        self.assertEqual(
    ...
    def test_import_csv(self):
        ...
        data = [("zaizee", 1, None), ("huey", 2, "foo"), ("mickey", 3, "baze")]
        buf = StringIO()
