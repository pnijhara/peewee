...
class Course(TestModel):
    ...
    students = ManyToManyField(
        Student, through_model=StudentCourseProxy, backref="courses"
    )
...
class TestModelToDict(ModelTestCase):
    ...
    def test_simple(self):
        with self.assertQueryCount(0):
            self.assertEqual(
                model_to_dict(self.user), {"id": self.user.id, "username": "peewee"}
            )
    ...
    def test_simple_recurse(self):
        ...
        with self.assertQueryCount(0):
            self.assertEqual(
                model_to_dict(tweet, recurse=False),
                {"id": tweet.id, "content": tweet.content, "user": self.user.id},
            )
    ...
    def test_simple_backref(self):
        with self.assertQueryCount(1):
            self.assertEqual(
                ...
                {"id": self.user.id, "tweets": [], "username": "peewee"},
            )
        ...
        with self.assertQueryCount(2):
            self.assertEqual(
                model_to_dict(self.user, backrefs=True),
                {
                    "id": self.user.id,
                    "username": "peewee",
                    "tweets": [{"id": tweet.id, "content": "t0", "tweettag_set": []}],
                },
            )
    ...
    def test_recurse_and_backrefs(self):
        ...
        with self.assertQueryCount(1):
            self.assertEqual(
                model_to_dict(tweet, backrefs=True),
                {
                    "id": tweet.id,
                    "content": "t0",
                    "tweettag_set": [],
    ...
    def test_recursive_fk(self):
        ...
        with self.assertQueryCount(0):
            for recurse in (True, False):
                self.assertEqual(
                    model_to_dict(root, recurse=recurse),
                    {"name": "root", "parent": None},
                )

        with self.assertQueryCount(1):
            self.assertEqual(
                model_to_dict(root, backrefs=True),
                {"name": "root", "parent": None, "children": [{"name": "child"}]},
            )
        ...
        with self.assertQueryCount(1):
            self.assertEqual(
                model_to_dict(child, backrefs=True),
                {
                    "name": "child",
                    "parent": {"name": "root"},
        ...
        with self.assertQueryCount(0):
            self.assertEqual(
                model_to_dict(child, backrefs=False),
    ...
    def test_manytomany_field(self):
        ...
        rows = [
            model_to_dict(gallery, backrefs=True, manytomany=True) for gallery in query
        ]
        self.assertEqual(
            rows,
            [
    ...
    def test_manytomany_deferred(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                },
                {
                    "id": 2,
                    "name": "s2",
                    "courses": [{"id": 2, "name": "cb"}, {"id": 4, "name": "cd"}],
                },
        ...
        self.assertEqual(
            data,
            [
                {"id": 1, "name": "ca", "students": [{"id": 1, "name": "s1"}]},
                {
                    "id": 2,
                    "name": "cb",
                    "students": [{"id": 1, "name": "s1"}, {"id": 2, "name": "s2"}],
                },
                {"id": 3, "name": "cc", "students": [{"id": 1, "name": "s1"}]},
                {"id": 4, "name": "cd", "students": [{"id": 2, "name": "s2"}]},
            ],
        )

    def test_recurse_max_depth(self):
        t0, t1, t2 = [Tweet.create(user=self.user, content="t%s" % i) for i in range(3)]
            ...
            data,
            {
                ...
                "tweets": [
                    ...
                    },
                    {
                        "id": t1.id,
                        "content": "t1",
                        "tweettag_set": [{"tag": {"tag": "tag1", "id": tag1.id}},],
                    },
        ...
        data = model_to_dict(self.user, recurse=True, backrefs=True, max_depth=2)
        self.assertEqual(
            data,
            {
                "id": self.user.id,
                ...
                "tweets": [
                    {
                        "id": t0.id,
                        "content": "t0",
                        "tweettag_set": [{"tag": tag0.id}, {"tag": tag1.id},],
                    },
                    {"id": t1.id, "content": "t1", "tweettag_set": [{"tag": tag1.id},]},
                    {"id": t2.id, "content": "t2", "tweettag_set": []},
                ],
            },
        )

        data = model_to_dict(self.user, recurse=True, backrefs=True, max_depth=1)
        self.assertEqual(
            data,
            {
                "id": self.user.id,
        ...
        self.assertEqual(
            model_to_dict(self.user, recurse=True, backrefs=True, max_depth=0),
            {"id": self.user.id, "username": "peewee"},
        )

    def test_only(self):
        username_dict = {"username": "peewee"}
        self.assertEqual(model_to_dict(self.user, only=[User.username]), username_dict)

        self.assertEqual(
            model_to_dict(self.user, backrefs=True, only=[User.username]), username_dict
        )

        tweet = Tweet.create(user=self.user, content="t0")
        tweet_dict = {"content": "t0", "user": {"username": "peewee"}}
        field_list = [Tweet.content, Tweet.user, User.username]
        self.assertEqual(model_to_dict(tweet, only=field_list), tweet_dict)
        self.assertEqual(
            model_to_dict(tweet, backrefs=True, only=field_list), tweet_dict
        )

        tweet_dict["user"] = self.user.id
        self.assertEqual(
            model_to_dict(tweet, backrefs=True, recurse=False, only=field_list),
            tweet_dict,
        )

    def test_exclude(self):
        self.assertEqual(
            model_to_dict(self.user, exclude=[User.id]), {"username": "peewee"}
        )

        # Exclude the foreign key using FK field and backref.
        self.assertEqual(
            model_to_dict(self.user, backrefs=True, exclude=[User.id, Tweet.user]),
            {"username": "peewee"},
        )
        self.assertEqual(
            model_to_dict(self.user, backrefs=True, exclude=[User.id, User.tweets]),
            {"username": "peewee"},
        )
        ...
        self.assertEqual(
            model_to_dict(tweet, backrefs=True, exclude=fields), {"content": "t0"}
        )
        fields[-1] = User.id
        self.assertEqual(
            model_to_dict(tweet, backrefs=True, exclude=fields),
    ...
    def test_extra_attrs(self):
        with self.assertQueryCount(0):
            extra = ["name_hash"]
            self.assertEqual(
                model_to_dict(self.user, extra_attrs=extra),
                {"id": self.user.id, "username": "peewee", "name_hash": 5},
            )

        with self.assertQueryCount(0):
            self.assertRaises(
                AttributeError, model_to_dict, self.user, extra_attrs=["xx"]
            )
    ...
    def test_fields_from_query(self):
        ...
        query = (
            User.select(User.username, fn.COUNT(Tweet.id).alias("ct"))
        ...
        with self.assertQueryCount(1):
            u0, u1, u2 = list(query)
            self.assertEqual(
                model_to_dict(u0, fields_from_query=query), {"username": "u0", "ct": 1}
            )
            self.assertEqual(
                model_to_dict(u2, fields_from_query=query), {"username": "u2", "ct": 3}
            )

        query = (
            Tweet.select(Tweet, User, SQL("1337").alias("magic"))
            .join(User)
            .order_by(Tweet.id)
            .limit(1)
        )
        with self.assertQueryCount(1):
            (tweet,) = query
            self.assertEqual(
                model_to_dict(tweet, fields_from_query=query),
                {
                    "id": tweet.id,
                    "content": "u0-0",
                    "magic": 1337,
            ...
            self.assertEqual(
                model_to_dict(
                    tweet, fields_from_query=query, exclude=[User.id, Tweet.id]
                ),
                {"magic": 1337, "content": "u0-0", "user": {"username": "u0"}},
            )
    ...
    def test_only_backref(self):
        ...
        data = model_to_dict(
            self.user, backrefs=True, only=[User.username, User.tweets, Tweet.content]
        )
        if "tweets" in data:
        ...
        self.assertEqual(
            ...
            {
                "username": "peewee",
                "tweets": [{"content": "0"}, {"content": "1"}, {"content": "2"}],
            },
        )
    ...
    def test_model_to_dict_disabled_backref(self):
        ...
        self.assertEqual(services, [{"id": 1, "name": "ssh"}, {"id": 2, "name": "vpn"}])
...
class TestDictToModel(ModelTestCase):
    ...
    def test_backrefs(self):
        data = {
            "id": self.user.id,
            "username": "peewee",
            "tweets": [{"id": 1, "content": "t1"}, {"id": 2, "content": "t2"},],
        }
    ...
    def test_unknown_attributes(self):
        data = {"id": self.user.id, "username": "peewee", "xx": "does not exist"}
...
class TestResolveMultiModelQuery(ModelTestCase):
    ...
    def test_resolve_multimodel_query(self):
        ...
        expected = [
            MMA(key="k0", value=0),
            MMA(key="k1", value=1),
            MMB(key="k10", value=99),
            MMB(key="k11", value=99),
            MMC(key="k20", value=20),
            MMC(key="k21", value=21),
        ]
