...
class TestPrefetch(ModelTestCase):
    ...
    def create_test_data(self):
        data = {
            ...
            "mickey": (("woof", ()), ("bark", ("bark-1", "bark-2"))),
            "zaizee": (),
        }
        ...
        Like.create(
            note=Note.get(Note.content == "meow"),
            person=Person.get(Person.name == "mickey"),
        )
        Like.create(
            note=Note.get(Note.content == "woof"),
    ...
    def test_prefetch_simple(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                ("mickey", [("bark", ["bark-1", "bark-2"]), ("woof", [])]),
                ("zaizee", []),
            ],
        )
    ...
    def test_prefetch_reverse_with_parent_join(self):
        with self.assertQueryCount(2):
            ...
            accum = [
                (note.person.name, note.content, [item.content for item in note.items])
                for note in query
            ]
            self.assertEqual(
                accum,
    ...
    def test_prefetch_multi_depth(self):
        ...
        likes = Like.select(Like, LikePerson.name).join(
            LikePerson, on=(Like.person == LikePerson.id)
        )
    ...
    def test_prefetch_with_group_by(self):
        people = (
            Person.select(Person, fn.COUNT(Note.id).alias("note_count"))
    ...
    def test_prefetch_self_join(self):
        ...
        with self.assertQueryCount(2):
            ...
            names_and_children = [
                (cat.name, [child.name for child in cat.children]) for cat in query
            ]
    ...
    def test_prefetch_specific_model(self):
        ...
        with self.assertQueryCount(3):
            ...
            notes = Note.select().order_by(Note.content)
            likes = (
                Like.select(Like, NoteAlias.content)
    ...
    def test_multiple_foreign_keys(self):
        ...
        with self.assertQueryCount(2):
            ...
            assertRelationships(
                hp.related_to, [{"id": r1.id, "from_person": c.id, "to_person": h.id}]
            )
            ...
            assertRelationships(
                zp.related_to, [{"id": r2.id, "from_person": c.id, "to_person": z.id}]
            )
        ...
        with self.assertQueryCount(3):
            people = (
                Person.select().where(Person.name != "mickey").order_by(Person.name)
            )
            relationships = Relationship.select().order_by(Relationship.id)
            PA = Person.alias()
        ...
        with self.assertQueryCount(2):
            ...
            rels = (
                Relationship.select(Relationship, PA)
    ...
    def test_prefetch_through_manytomany(self):
        ...
        self.assertEqual(
            accum,
            [("huey", ["woof"]), ("mickey", ["meow"]), ("zaizee", ["meow", "woof"]),],
        )
