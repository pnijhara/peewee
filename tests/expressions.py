...
class TestValueConversion(ModelTestCase):
    ...
    def test_value_conversion(self):
        ...
        self.assertSQL(
            insert, ('INSERT INTO "upper_model" ("name") VALUES (UPPER(?))'), ["huey"]
        )
        uid = insert.execute()
        ...
        update = UpperModel.update({UpperModel.name: "zaizee"}).where(
            UpperModel.id == uid
        )
        self.assertSQL(
            update,
        ...
        self.assertSQL(
            delete,
            ('DELETE FROM "upper_model" ' 'WHERE ("upper_model"."name" = UPPER(?))'),
            ["zaizee"],
        )
    ...
    def test_value_conversion_query(self):
        ...
        # Join on sub-query.
        query = UpperModel.select().join(subq, on=(UpperModel.name == subq.c.name))
        self.assertSQL(
            query,
            (
                'SELECT "t1"."id", "t1"."name" FROM "upper_model" AS "t1" '
    ...
    def test_having_clause(self):
        query = (
            UpperModel.select(UpperModel.name, fn.COUNT(UpperModel.id).alias("ct"))
