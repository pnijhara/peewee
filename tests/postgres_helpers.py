...
class BaseJsonFieldTestCase(object):
    ...
    def test_joining_on_json_key(self):
        ...
        query = (
            self.M.select()
            .join(self.N, on=(self.N.data == self.M.data["baze"]["nugget"]))
            .order_by(self.M.id)
        )
    ...
    def test_json_cast(self):
        ...
        query = (
            self.M.select(Cast(self.M.data["foo"]["bar"], "float") * 1.5)
            .order_by(self.M.id)
            .tuples()
        )
...
class BaseBinaryJsonFieldTestCase(BaseJsonFieldTestCase):
    ...
    def test_concat_data(self):
        ...
        assertData(
            {"k1": "v1-x", "k3": [2, 3, 4], "k4": {"x4": "y4"}},
            {"k1": "v1-x", "k2": "v2", "k3": [2, 3, 4], "k4": {"x4": "y4"}},
        )
    ...
    def test_update_data_inplace(self):
        ...
        self.M.update(
            data=self.M.data.concat({"k1": {"x2": "y2"}, "k3": "v3"})
        ).execute()
    ...
    def test_selecting(self):
        self._create_test_data()
        query = self.M.select(self.M.data["k3"]["k4"].as_json().alias("k3k4")).order_by(
            self.M.id
        )
        ...
        query = (
            self.M.select(self.M.data[0].as_json(), self.M.data[2].as_json())
            .order_by(self.M.id)
            .tuples()
        )
        self.assertEqual(
    ...
    def test_conflict_update(self):
        ...
        iq = self.M.insert(id=b1.id, data={"k1": "v1-x"}).on_conflict(
            "update", conflict_target=[self.M.id], update={self.M.data: {"k1": "v1-z"}}
        )
        b1_id_db = iq.execute()
        ...
        iq = self.M.insert(id=b1.id, data={"k1": "v1-y"}).on_conflict(
            "update", conflict_target=[self.M.id], update={"data": {"k1": "v1-w"}}
        )
        b1_id_db = iq.execute()
        self.assertEqual(b1.id, b1_id_db)
