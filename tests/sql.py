...
class TestSelectQuery(BaseTestCase):
    def test_select(self):
        query = User.select(User.c.id, User.c.username).where(User.c.username == "foo")
        self.assertSQL(
            query,
            (
    ...
    def test_select_extend(self):
        ...
        self.assertSQL(
            query, ('SELECT "t1"."id", "t1"."username" FROM "users" AS "t1"'), []
        )

        query = query.select(User.c.username, User.c.is_admin)
        self.assertSQL(
            query, ('SELECT "t1"."username", "t1"."is_admin" FROM "users" AS "t1"'), []
        )
    ...
    def test_select_explicit_columns(self):
        query = Person.select().where(Person.dob < datetime.date(1980, 1, 1))
        self.assertSQL(
            query,
            (
    ...
    def test_select_in_list_of_values(self):
        ...
        for names in names_vals:
            query = Person.select().where(Person.name.in_(names))
    ...
    def test_select_subselect_function(self):
        ...
        exists = fn.EXISTS(Tweet.select(Tweet.c.id).where(Tweet.c.user_id == User.c.id))
    ...
    def test_subquery_in_select_sql(self):
        ...
        query = Tweet.select(
            Tweet.c.content, Tweet.c.user_id.in_(subq).alias("is_huey")
        )
        self.assertSQL(
            query,
            (
        ...
        self.assertSQL(
            ...
            (
                'SELECT "t1"."content", ("t1"."user_id" IN ('
                'SELECT "t2"."id" FROM "users" AS "t2" WHERE ("t2"."username" = ?)'
                ') AS "sq") AS "is_huey" FROM "tweets" AS "t1"'
            ),
            ["huey"],
        )
    ...
    def test_from_clause(self):
        query = (
            Note.select(Note.content, Person.name)
            .from_(Note, Person)
    ...
    def test_from_query(self):
        ...
        query = Person.select(Person.name).from_(inner.alias("i1"))
        self.assertSQL(
            query,
            (
                'SELECT "t1"."name" '
        ...
        query = Person.select(inner.c.name).from_(inner).order_by(inner.c.name)
        self.assertSQL(
            query,
            (
                'SELECT "i1"."name" '
    ...
    def test_multi_join(self):
        ...
        query = (
            Like.select(Tweet.c.content, User.c.username, LikeUser.c.username)
    ...
    def test_correlated_subquery(self):
        ...
        query = Employee.select(Employee.id, Employee.name).where(
            Employee.salary
            > (EA.select(fn.AVG(EA.salary)).where(EA.dept == Employee.dept))
        )
        self.assertSQL(
            query,
            (
    ...
    def test_limit(self):
        ...
        self.assertSQL(base.limit(None), ('SELECT "t1"."id" FROM "users" AS "t1"'), [])
        self.assertSQL(
            base.limit(10), ('SELECT "t1"."id" FROM "users" AS "t1" LIMIT ?'), [10]
        )
        self.assertSQL(
            base.limit(10).offset(3),
            ('SELECT "t1"."id" FROM "users" AS "t1" ' "LIMIT ? OFFSET ?"),
            [10, 3],
        )
        self.assertSQL(
            base.limit(0), ('SELECT "t1"."id" FROM "users" AS "t1" LIMIT ?'), [0]
        )

        self.assertSQL(
            base.offset(3),
            ('SELECT "t1"."id" FROM "users" AS "t1" OFFSET ?'),
            [3],
            limit_max=None,
        )
    ...
    def test_simple_join(self):
        query = (
            User.select(User.c.id, User.c.username, fn.COUNT(Tweet.c.id).alias("ct"))
    ...
    def test_subquery(self):
        inner = Tweet.select(fn.COUNT(Tweet.c.id).alias("ct")).where(
            Tweet.c.user == User.c.id
        )
        query = User.select(User.c.username, inner.alias("iq")).order_by(
            User.c.username
        )
        self.assertSQL(
            query,
    ...
    def test_subquery_in_expr(self):
        ...
        query = Challenge.select((Challenge.c.points / subq).alias("score")).order_by(
            SQL("score")
        )
        self.assertSQL(
            query,
            (
                ...
                'FROM "challenge" AS "t1" ORDER BY score'
            ),
            [1],
        )
    ...
    def test_user_defined_alias(self):
        UA = User.alias("alt")
        query = (
            User.select(User.c.id, User.c.username, UA.c.nuggz)
    ...
    def test_simple_cte(self):
        ...
        query = User.select(User.c.username).where(User.c.id.in_(cte)).with_cte(cte)
        self.assertSQL(
            query,
            (
    ...
    def test_two_ctes(self):
        ...
        query = (
            User.select(c1.c.id, c2.c.username)
            .where((c1.c.id == User.c.id) & (c2.c.username == User.c.username))
            .with_cte(c1, c2)
        )
        self.assertSQL(
            query,
    ...
    def test_select_from_cte(self):
        ...
        query = Select((c1, c2), (c1.c.username, c2.c.username)).with_cte(c1, c2)
        self.assertSQL(
            query,
            (
                'WITH "c1" AS ('
                ...
                'SELECT "t1"."username" FROM "users" AS "t1" '
                'WHERE ("t1"."is_staff" = ?)) '
                'SELECT "c1"."username", "c2"."username" FROM "c1", "c2"'
            ),
            [1, 1],
        )

    def test_materialize_cte(self):
        cases = ((True, "MATERIALIZED "), (False, "NOT MATERIALIZED "), (None, ""))
        for materialized, clause in cases:
            cte = User.select(User.c.id).cte("user_ids", materialized=materialized)
    ...
    def test_fibonacci_cte(self):
        q1 = Select(
            columns=(
                Value(1).alias("n"),
                Value(0).alias("fib_n"),
                Value(1).alias("next_fib_n"),
            )
        ).cte("fibonacci", recursive=True)
        n = (q1.c.n + 1).alias("n")
        rterm = (
            Select(columns=(n, q1.c.next_fib_n, q1.c.fib_n + q1.c.next_fib_n))
            .from_(q1)
            .where(n < 10)
        )

        cases = ((q1.union_all, "UNION ALL"), (q1.union, "UNION"))
    ...
    def test_cte_subquery_in_expression(self):
        ...
        qexpr = (
            Order.select(Order.id)
            .join(cte, on=(Order.id == cte.c.max_id))
            .with_cte(cte)
        )
        query = Item.select(Item.id, Item.order_id, Item.description).where(
            Item.order_id.in_(qexpr)
        )
        self.assertSQL(
            query,
            (
    ...
    def test_complex_select(self):
        Order = Table("orders", columns=("region", "amount", "product", "quantity"))

        regional_sales = (
            Order.select(Order.region, fn.SUM(Order.amount).alias("total_sales"))
            .group_by(Order.region)
            .cte("regional_sales")
        )

        top_regions = (
            regional_sales.select(regional_sales.c.region)
            .where(
                regional_sales.c.total_sales
                > (regional_sales.select(fn.SUM(regional_sales.c.total_sales) / 10))
            )
            .cte("top_regions")
        )
        ...
        query = (
            ...
            .where(Order.region << top_regions.select(top_regions.c.region))
    ...
    def test_compound_operations(self):
        admin = User.select(User.c.username, Value("admin").alias("role")).where(
            User.c.is_admin == True
        )
        editors = User.select(User.c.username, Value("editor").alias("role")).where(
            User.c.is_editor == True
        )
    ...
    def test_compound_parentheses_handling(self):
        admin = (
            User.select(User.c.username, Value("admin").alias("role"))
            .where(User.c.is_admin == True)
            .order_by(User.c.id.desc())
            .limit(3)
        )
        editors = (
            User.select(User.c.username, Value("editor").alias("role"))
        ...
        for csq_setting in (1, 2):
            self.assertSQL(
                ...
                [2, 7],
                compound_select_parentheses=csq_setting,
            )
    ...
    def test_compound_select_as_subquery(self):
        ...
        outer = union.select_from(
            union.c.foo, fn.COUNT(union.c.foo).alias("ct")
    ...
    def test_join_on_query(self):
        ...
        query = Tweet.select(Tweet.c.content).join(
            inner, on=(Tweet.c.user_id == inner.c.id)
        )
        self.assertSQL(
            query,
            (
    ...
    def test_all_clauses(self):
        ...
        query = (
            User.select(User.c.username, count)
            .join(Tweet, JOIN.LEFT_OUTER, on=(User.c.id == Tweet.c.user_id))
    ...
    def test_order_by_collate(self):
        query = User.select(User.c.username).order_by(
            User.c.username.asc(collation="binary")
        )
        self.assertSQL(
            query,
            (
    ...
    def test_order_by_nulls(self):
        query = User.select(User.c.username).order_by(User.c.ts.desc(nulls="LAST"))
        self.assertSQL(
            query,
            (
                'SELECT "t1"."username" FROM "users" AS "t1" '
                'ORDER BY "t1"."ts" DESC NULLS LAST'
            ),
            [],
            nulls_ordering=True,
        )
        ...
        query = User.select(User.c.username).order_by(User.c.ts.desc(nulls="first"))
        self.assertSQL(
            query,
            (
                'SELECT "t1"."username" FROM "users" AS "t1" '
                'ORDER BY "t1"."ts" DESC NULLS first'
            ),
            [],
            nulls_ordering=True,
        )
    ...
    def test_in_value_representation(self):
        query = User.select(User.c.id).where(User.c.username.in_(["foo", "bar", "baz"]))
        self.assertSQL(
            query,
            (
                'SELECT "t1"."id" FROM "users" AS "t1" '
    ...
    def test_tuple_comparison(self):
        ...
        query = Person.select(Person.id).where(name_dob == ("foo", "2017-01-01"))
        expected = (
            'SELECT "t1"."id" FROM "person" AS "t1" '
        ...
        query = Person.select(Person.id).where(name_dob == Tuple("foo", "2017-01-01"))
        self.assertSQL(query, expected, ["foo", "2017-01-01"])

    def test_tuple_comparison_subquery(self):
        PA = Person.alias("pa")
        subquery = PA.select(PA.name, PA.id).where(PA.name != "huey")

        query = Person.select(Person.name).where(
            Tuple(Person.name, Person.id).in_(subquery)
        )
        self.assertSQL(
            query,
            (
    ...
    def test_empty_in(self):
        ...
        self.assertSQL(
            query, ('SELECT "t1"."id" FROM "users" AS "t1" ' "WHERE (0 = 1)"), []
        )

        query = User.select(User.c.id).where(User.c.username.not_in([]))
        self.assertSQL(
            query, ('SELECT "t1"."id" FROM "users" AS "t1" ' "WHERE (1 = 1)"), []
        )
    ...
    def test_add_custom_op(self):
        ...
        Stat = Table("stats")
        query = Stat.select(fn.COUNT(Stat.c.id)).where(mod(Stat.c.index, 10) == 0)
        self.assertSQL(
            query,
            (
    ...
    def test_where_convert_to_is_null(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'FROM "notes" AS "t1" WHERE ("t1"."user_id" IS ?)'
            ),
            [None],
        )
...
class TestInsertQuery(BaseTestCase):
    def test_insert_simple(self):
        query = User.insert(
            {User.c.username: "charlie", User.c.superuser: False, User.c.admin: True}
        )
        self.assertSQL(
            query,
            (
    ...
    def test_replace_sqlite(self):
        query = User.replace({User.c.username: "charlie", User.c.superuser: False})
        self.assertSQL(
            query,
            (
    ...
    def test_replace_mysql(self):
        ...
        self.assertSQL(
            query,
            ('REPLACE INTO "users" ("superuser", "username") ' "VALUES (?, ?)"),
            [False, "charlie"],
        )
    ...
    def test_insert_single_value_query(self):
        ...
        insert = Note.insert({Note.person_id: query, Note.content: "hello"})
        self.assertSQL(
            insert,
            (
    ...
    def test_insert_returning(self):
        query = Person.insert(
            {Person.name: "zaizee", Person.dob: datetime.date(2000, 1, 2)}
...
class TestUpdateQuery(BaseTestCase):
    ...
    def test_update_subquery(self):
        count = fn.COUNT(Tweet.c.id).alias("ct")
        subquery = (
            User.select(User.c.id, count)
            .join(Tweet, on=(Tweet.c.user_id == User.c.id))
            .group_by(User.c.id)
            .having(count > 100)
        )
        query = User.update({User.c.muted: True, User.c.counter: 0}).where(
            User.c.id << subquery
        )
        self.assertSQL(
            query,
    ...
    def test_update_value_subquery(self):
        subquery = Tweet.select(fn.MAX(Tweet.c.id)).where(Tweet.c.user_id == User.c.id)
        query = User.update({User.c.last_tweet_id: subquery}).where(
            User.c.last_tweet_id.is_null(True)
        )
        self.assertSQL(
            query,
            (
    ...
    def test_update_from(self):
        ...
        query = (
            User.update(username=vl.c.username).from_(vl).where(User.c.id == vl.c.id)
        )
        self.assertSQL(
            query,
            (
        ...
        query = (
            User.update({User.c.username: subq.c.username})
            .from_(subq)
    ...
    def test_update_returning(self):
        query = (
            User.update({User.c.is_admin: True})
...
class TestDeleteQuery(BaseTestCase):
    def test_delete_query(self):
        query = User.delete().where(User.c.username != "charlie").limit(3)
        self.assertSQL(
            query,
    ...
    def test_delete_subquery(self):
        ...
        query = User.delete().where(User.c.id << subquery)
        self.assertSQL(
            query,
            (
                'DELETE FROM "users" '
    ...
    def test_delete_cte(self):
        cte = User.select(User.c.id).where(User.c.admin == True).cte("u")
        query = User.delete().where(User.c.id << cte.select(cte.c.id)).with_cte(cte)
        self.assertSQL(
            query,
            (
                'WITH "u" AS '
                ...
                'DELETE FROM "users" '
                'WHERE ("users"."id" IN (SELECT "u"."id" FROM "u"))'
            ),
            [True],
        )

    def test_delete_returning(self):
        query = User.delete().where(User.c.id > 2).returning(User.c.username)
        self.assertSQL(
            query,
            (
                'DELETE FROM "users" '
...
class TestWindowFunctions(BaseTestCase):
    ...
    def test_ordered_unpartitioned(self):
        query = Register.select(
            Register.value, fn.RANK().over(order_by=[Register.value])
        )
        self.assertSQL(
            query,
            (
    ...
    def test_ordered_partitioned(self):
        query = Register.select(
            ...
            .over(order_by=Register.id, partition_by=Register.category)
            .alias("rsum"),
        )
        self.assertSQL(
            query,
    ...
    def test_empty_over(self):
        query = Register.select(
            Register.value, fn.LAG(Register.value, 1).over()
    ...
    def test_frame_types(self):
        def assertFrame(over_kwargs, expected):
            query = Register.select(
                Register.value, fn.SUM(Register.value).over(**over_kwargs)
            )
        ...
        assertFrame(
            {"start": Window.preceding(), "frame_type": Window.RANGE},
            "RANGE UNBOUNDED PRECEDING",
        )
        assertFrame(
            {"start": Window.preceding(), "frame_type": Window.ROWS},
        ...
        assertFrame(
            {"frame_type": Window.RANGE, "order_by": Register.value},
            'ORDER BY "t1"."value" RANGE UNBOUNDED PRECEDING',
        )
        assertFrame(
            {"frame_type": Window.ROWS, "order_by": Register.value},
            'ORDER BY "t1"."value" ROWS UNBOUNDED PRECEDING',
        )
        assertFrame({"partition_by": Register.category}, 'PARTITION BY "t1"."category"')
        assertFrame(
            {"frame_type": Window.RANGE, "partition_by": Register.category,},
            'PARTITION BY "t1"."category" RANGE UNBOUNDED PRECEDING',
        )
        assertFrame(
            {"frame_type": Window.ROWS, "partition_by": Register.category,},
    ...
    def test_running_total(self):
        ...
        query = EventLog.select(
            EventLog.timestamp, EventLog.data, w.alias("elapsed")
        ...
        w = fn.SUM(EventLog.timestamp).over(
            order_by=[EventLog.timestamp], partition_by=[EventLog.data]
        )
        query = EventLog.select(
            EventLog.timestamp, EventLog.data, w.alias("elapsed")
        ).order_by(EventLog.timestamp)
    ...
    def test_named_window(self):
        ...
        query = Register.select(
            Register.category, Register.value, fn.AVG(Register.value).over(window)
        ).window(window)
        ...
        window = Window(
            partition_by=[Register.category], order_by=[Register.value.desc()]
        )
        query = Register.select(Register.value, fn.RANK().over(window)).window(window)
        self.assertSQL(
            query,
            (
    ...
    def test_multiple_windows(self):
        ...
        query = Register.select(
            Register.value, fn.AVG(Register.value).over(w1), fn.RANK().over(w2)
        ).window(w1, w2)
        self.assertSQL(
            query,
            (
    ...
    def test_reuse_window(self):
        ...
        window = Window(partition_by=[EventLog.key], order_by=[EventLog.timestamp])
        query = (
            EventLog.select(
                EventLog.timestamp,
                EventLog.key,
    ...
    def test_filter_clause(self):
        ...
        query = Register.select(Register.category, Register.value, condsum).order_by(
            Register.category
        )
        self.assertSQL(
            query,
    ...
    def test_window_in_orderby(self):
        ...
        query = (
            Register.select().window(w).order_by(fn.FIRST_VALUE(Register.id).over(w))
        )
        self.assertSQL(
            query,
            (
        ...
        fv = fn.FIRST_VALUE(Register.id).over(
            partition_by=[Register.value], order_by=[Register.id]
        )
    ...
    def test_window_extends(self):
        ...
        query = Tbl.select(fn.GROUP_CONCAT(Tbl.c).over(w4)).window(w1, w2, w3, w4)
        self.assertSQL(
            query,
            (
    ...
    def test_window_frametypes(self):
        ...
        for method, arg, sql in fts:
            ...
            self.assertSQL(
                Tbl.select(fn.SUM(Tbl.c).over(w)).window(w),
                (
            ...
            query = Tbl.select(fn.SUM(Tbl.c).over(order_by=[Tbl.b + 1], frame_type=arg))
            self.assertSQL(
                query,
                (
                    'SELECT SUM("t1"."c") OVER (ORDER BY ("t1"."b" + ?) '
                    '%s UNBOUNDED PRECEDING) FROM "tbl" AS "t1"'
                )
                % sql,
                [1],
            )
    ...
    def test_window_frame_exclusion(self):
        Tbl = Table("tbl", ("b", "c"))
        fts = (
            (Window.CURRENT_ROW, "CURRENT ROW"),
            (Window.TIES, "TIES"),
    ...
    def test_filter_window(self):
        ...
        Tbl = Table("tbl", ("a", "c"))
        win = Window(
            partition_by=fn.COALESCE(Tbl.a, ""),
            frame_type=Window.RANGE,
            start=Window.CURRENT_ROW,
            end=Window.following(),
        ...
        query = Tbl.select(
            fn.SUM(Tbl.c).filter(Tbl.c < 5).over(win),
            fn.RANK().over(win),
...
class TestCaseFunction(BaseTestCase):
    def test_case_function(self):
        ...
        query = NameNum.select(
            NameNum.name,
            Case(NameNum.number, ((1, "one"), (2, "two")), "?").alias("num_str"),
        )
        self.assertSQL(
            query,
            (
        ...
        query = NameNum.select(
            NameNum.name,
            Case(
                None, ((NameNum.number == 1, "one"), (NameNum.number == 2, "two")), "?"
            ),
        )
        self.assertSQL(
            query,
...
class TestSelectFeatures(BaseTestCase):
    def test_reselect(self):
        ...
        self.assertSQL(
            query,
            ('SELECT "t1"."id", "t1"."name", "t1"."dob" ' 'FROM "person" AS "t1"'),
            [],
        )

    def test_distinct_on(self):
        query = (
            Note.select(Person.name, Note.content)
            .join(Person, on=(Note.person_id == Person.id))
        ...
        query = Person.select(Person.name).distinct(Person.name)
        self.assertSQL(
            query,
            ('SELECT DISTINCT ON ("t1"."name") "t1"."name" ' 'FROM "person" AS "t1"'),
            [],
        )

    def test_distinct(self):
        query = Person.select(Person.name).distinct()
        self.assertSQL(query, 'SELECT DISTINCT "t1"."name" FROM "person" AS "t1"', [])

    def test_distinct_count(self):
        query = Person.select(fn.COUNT(Person.name.distinct()))
        self.assertSQL(
            query, ('SELECT COUNT(DISTINCT "t1"."name") FROM "person" AS "t1"'), []
        )

    def test_filtered_count(self):
        filtered_count = fn.COUNT(Person.name).filter(
            Person.dob < datetime.date(2000, 1, 1)
        )
    ...
    def test_ordered_aggregate(self):
        ...
        agg = fn.string_agg(Person.name.concat("-x"), ",").order_by(
            Person.name.desc(), Person.dob.asc()
        )
        self.assertSQL(
            Person.select(agg),
            (
        ...
        self.assertSQL(
            Person.select(agg),
            ('SELECT string_agg(("t1"."name" || ?), ?) ' 'FROM "person" AS "t1"'),
            ["-x", ","],
        )

    def test_for_update(self):
        query = Person.select().where(Person.name == "charlie").for_update()
        self.assertSQL(
            query,
            (
                'SELECT "t1"."id", "t1"."name", "t1"."dob" '
    ...
    def test_for_update_nested(self):
        ...
        query = Person.delete().where(Person.id.in_(subq))
        self.assertSQL(
            query,
            (
    ...
    def test_parentheses(self):
        query = Person.select(fn.MAX(fn.IFNULL(1, 10) * 151, fn.IFNULL(None, 10)))
        self.assertSQL(
            query,
            ("SELECT MAX((IFNULL(?, ?) * ?), IFNULL(?, ?)) " 'FROM "person" AS "t1"'),
            [1, 10, 151, None, 10],
        )

        query = Person.select(Person.name).where(
            fn.EXISTS(User.select(User.c.id).where(User.c.username == Person.name))
        )
        self.assertSQL(
            query,
            (
...
class TestOnConflictSqlite(BaseTestCase):
    ...
    def test_replace(self):
        ...
        self.assertSQL(
            query, ('INSERT OR REPLACE INTO "person" ("name") VALUES (?)'), ["huey"]
        )

    def test_ignore(self):
        query = Person.insert(name="huey").on_conflict("ignore")
        self.assertSQL(
            query, ('INSERT OR IGNORE INTO "person" ("name") VALUES (?)'), ["huey"]
        )
...
class TestOnConflictMySQL(BaseTestCase):
    ...
    def test_replace(self):
        query = Person.insert(name="huey").on_conflict("replace")
        self.assertSQL(query, ('REPLACE INTO "person" ("name") VALUES (?)'), ["huey"])

    def test_ignore(self):
        query = Person.insert(name="huey").on_conflict("ignore")
        self.assertSQL(
            query, ('INSERT IGNORE INTO "person" ("name") VALUES (?)'), ["huey"]
        )

    def test_update(self):
        dob = datetime.date(2010, 1, 1)
        query = Person.insert(name="huey", dob=dob).on_conflict(
            preserve=(Person.dob,), update={Person.name: Person.name.concat("-x")}
        )
        self.assertSQL(
            query,
            (
        ...
        query = Person.insert(name="huey", dob=dob).on_conflict(preserve="dob")
        self.assertSQL(
            query,
            (
                'INSERT INTO "person" ("dob", "name") VALUES (?, ?) '
    ...
    def test_update_use_value_mariadb(self):
        ...
        dob = datetime.date(2010, 1, 1)
        query = Person.insert(name="huey", dob=dob).on_conflict(preserve=(Person.dob,))
    ...
    def test_where_not_supported(self):
        query = Person.insert(name="huey").on_conflict(
            preserve=(Person.dob,), where=(Person.name == "huey")
        )
        with self.assertRaisesCtx(ValueError):
            self.database.get_sql_context().parse(query)
...
class TestOnConflictPostgresql(BaseTestCase):
    ...
    def test_ignore(self):
        ...
        self.assertSQL(
            query,
            ('INSERT INTO "person" ("name") VALUES (?) ' "ON CONFLICT DO NOTHING"),
            ["huey"],
        )
    ...
    def test_conflict_target_or_constraint(self):
        ...
        query = KV.insert(key="k1", value="v1", extra="e1").on_conflict(
            conflict_target=[KV.key, KV.value], preserve=[KV.extra]
        )
        self.assertSQL(
            query,
            (
        ...
        query = KV.insert(key="k1", value="v1", extra="e1").on_conflict(
            conflict_constraint="kv_key_value", preserve=[KV.extra]
        )
        self.assertSQL(
            query,
            (
    ...
    def test_update(self):
        ...
        query = Person.insert(name="huey", dob=dob).on_conflict(
            conflict_target="name", preserve="dob"
        )
        self.assertSQL(
            query,
            (
    ...
    def test_conflict_target_partial_index(self):
        ...
        query = KVE.insert(data, columns).on_conflict(
...
class TestIndex(BaseTestCase):
    def test_simple_index(self):
        ...
        self.assertSQL(
            pidx, ('CREATE UNIQUE INDEX "person_name" ON "person" ("name")'), []
        )
    ...
    def test_str_cols(self):
        ...
        self.assertSQL(
            uidx, ('CREATE INDEX "users_info" ON "users" (username DESC, id)'), []
        )
