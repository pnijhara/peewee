...
class TestCySqliteHelpers(CyDatabaseTestCase):
    ...
    def test_update_hook(self):
        ...
        self.assertEqual(
            state,
            [("INSERT", "main", "register", 1), ("INSERT", "main", "register", 2)],
        )
...
class TestHashFunctions(CyDatabaseTestCase):
    ...
    def test_md5(self):
        ...
        query = (
            HUser.select(
                HUser.username, fn.SUBSTR(fn.SHA1(HUser.username), 1, 6).alias("sha")
            )
        ...
        self.assertEqual(
            query[:], [("charlie", "d8cd10"), ("huey", "89b31a"), ("zaizee", "b4dcf9")]
        )
...
class TestBackup(CyDatabaseTestCase):
    ...
    def test_backup_progress_error(self):
        ...
        other_db = CSqliteExtDatabase(":memory:")
        self.assertRaises(
            ValueError, self.database.backup, other_db, progress=broken_progress
        )
        other_db.close()
    ...
    def populate(self):
        ...
        with self.database.atomic():
            for i in "abcdefghijklmnopqrstuvwxyz":
                ...
                accum.extend(keys)
                self.execute(
                    "insert into register (data) values %s"
                    % ", ".join(["(?)"] * len(keys)),
                    *keys
                )
...
class TestDataTypesTableFunction(CyDatabaseTestCase):
    ...
    def test_data_types_table_function(self):
        ...
        cursor = self.database.execute_sql(
            "SELECT key, value " "FROM data_types() ORDER BY key"
        )
        self.assertEqual(
            cursor.fetchall(),
            [
