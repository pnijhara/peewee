...
class SqlCipherTestCase(CleanUpModelTestCase):
    ...
    def test_rekey(self):
        ...
        db2 = SqlCipherDatabase(
            db.database, passphrase="a new passphrase", pragmas=PRAGMAS
        )
...
config_db = SqlCipherDatabase(
    "peewee_test.dbc",
    pragmas={"kdf_iter": 1234, "cipher_page_size": 8192},
...
class SqlCipherExtTestCase(CleanUpModelTestCase):
    ...
    def test_fts(self):
        ...
        query = (
            FTSNote.select(FTSNote, FTSNote.rank().alias("score"))
    ...
    def test_func(self):
        ...
        query = (
            Note.select(Note.content, fn.shazam(Note.content).alias("shz"))
            .order_by(Note.id)
            .dicts()
        )
        results = list(query)
