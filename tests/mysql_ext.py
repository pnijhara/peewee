...
class TestMySQLConnector(ModelTestCase):
    ...
    def test_basic_operations(self):
        with self.database.atomic():
            charlie, huey, zaizee = [
                Person.create(first=f, last="leifer")
        ...
        self.assertEqual(
            [person.first for person in people], ["charlie", "huey", "zaizee"]
        )
    ...
    def test_match_expression(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'FROM "person" AS "t1" '
                'WHERE MATCH("t1"."first") AGAINST(?)'
            ),
            ["charlie"],
        )

        query = Person.select().where(
            Match((Person.first, Person.last), "huey AND zaizee", "IN BOOLEAN MODE")
        )
        self.assertSQL(
            query,
            (
