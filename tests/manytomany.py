...
class TestManyToManyBackrefBehavior(ModelTestCase):
    ...
    def setUp(self):
        ...
        huey, mickey, zaizee = [
            Student.create(name=name) for name in ("huey", "mickey", "zaizee")
        ]
...
class TestManyToMany(ModelTestCase):
        ...
        for username, notes in self.user_to_note.items():
            ...
            for note in notes:
                NoteUserThrough.create(
                    note=Note.get(Note.text == "note-%s" % note), user=user
                )
    ...
    def test_prefetch_notes(self):
        ...
        with self.assertQueryCount(3):
            gargie, huey, mickey, zaizee = prefetch(
                User.select().order_by(User.username), NoteUserThrough, Note
            )
    ...
    def test_prefetch_users(self):
        ...
        with self.assertQueryCount(3):
            n1, n2, n3, n4, n5 = prefetch(
                Note.select().order_by(Note.text), NoteUserThrough, User
            )
    ...
    def test_add(self):
        ...
        with self.assertQueryCount(1):
            huey.notes.add(
                Note.select().where(fn.substr(Note.text, 6, 1) << ["1", "3", "5"])
            )
        ...
        with self.assertQueryCount(2):
            n3.users.add(
                User.select().where(User.username != "gargie"), clear_existing=True
            )
    ...
    def test_remove(self):
        ...
        with self.assertQueryCount(1):
            huey.notes.remove(
                Note.select().where(Note.text << ["note-2", "note-4", "note-5"])
            )
