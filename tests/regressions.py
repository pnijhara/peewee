...
class CARef(TestModel):
    colalias = ForeignKeyField(
        ColAlias, backref="carefs", column_name="ca", object_id_name="colalias_id"
    )
...
class TestQueryAliasToColumnName(ModelTestCase):
    ...
    def test_alias_to_column_name(self):
        ...
        query = ColAlias.select(ColAlias.name.alias("pname")).order_by(ColAlias.name)
    ...
    def test_alias_overlap_with_join(self):
        query = (
            CARef.select(CARef, ColAlias.name.alias("pname"))
            .join(ColAlias)
            .order_by(ColAlias.name)
        )
        with self.assertQueryCount(1):
            self.assertEqual([r.colalias.pname for r in query], ["huey", "mickey"])

        # Note: we cannot alias the join to "ca", as this is the object-id
        # descriptor name.
        query = (
            CARef.select(CARef, ColAlias.name.alias("pname"))
            .join(ColAlias, on=(CARef.colalias == ColAlias.id).alias("ca"))
            .order_by(ColAlias.name)
        )
        with self.assertQueryCount(1):
...
class TestDeleteInstanceRegression(ModelTestCase):
    ...
    def test_delete_instance_regression(self):
        ...
        self.assertEqual(
            [d.d for d in DiD.select().order_by(DiD.d)], ["a3-b1-c-d", "a3-b2-c-d"]
        )
        self.assertEqual(
            [c.c for c in DiC.select().order_by(DiC.c)], ["a3-b1-c", "a3-b2-c"]
        )
        self.assertEqual(
            [b.b for b in DiB.select().order_by(DiB.b)], ["a3-b1", "a3-b2"]
        )
        self.assertEqual(
            [ba.b for ba in DiBA.select().order_by(DiBA.b)], ["a3-b1", "a3-b2"]
        )
...
class TestGithub1354(ModelTestCase):
    ...
    def test_get_or_create_self_referential_fk2(self):
        ...
        child, created = Category2.get_or_create(parent=parent, name="child", user=huey)
...
class TestInsertFromSQL(ModelTestCase):
    def setUp(self):
        ...
        self.database.execute_sql("create table if not exists user_src " "(name TEXT);")
...
class TestSubqueryFunctionCall(BaseTestCase):
    def test_subquery_function_call(self):
        Sample = Table("sample")
        SA = Sample.alias("s2")
        query = Sample.select(Sample.c.data).where(
            ~fn.EXISTS(SA.select(SQL("1")).where(SA.c.key == "foo"))
        )
        self.assertSQL(
            query,
            (
                ...
                "WHERE NOT EXISTS("
                'SELECT 1 FROM "sample" AS "s2" WHERE ("s2"."key" = ?))'
            ),
            ["foo"],
        )
...
class TestCrossJoin(ModelTestCase):
    ...
    def setUp(self):
        ...
        C.insert_many(
            [(1, 1, 1), (2, 1, 2), (3, 2, 1)], fields=[C.id, C.a, C.b]
        ).execute()

    def test_cross_join(self):
        query = (
            A.select(A.id.alias("aid"), B.id.alias("bid"))
            .join(B, JOIN.CROSS)
            .join(C, JOIN.LEFT_OUTER, on=((C.a == A.id) & (C.b == B.id)))
            .where(C.id.is_null())
            .order_by(A.id, B.id)
        )
...
class TestSubqueryInSelect(ModelTestCase):
    ...
    def test_subquery_in_select(self):
        ...
        query = Tweet.select(
            Tweet.content, Tweet.user.in_(subq).alias("is_huey")
        ).order_by(Tweet.content)
        self.assertEqual(
            [(r.content, r.is_huey) for r in query],
            [
                ("bark", False),
                ("hiss", True),
                ("meow", True),
...
class TestReturningIntegrationRegressions(ModelTestCase):
    ...
    def test_returning_integration_subqueries(self):
        ...
        subq = Tweet.select(fn.COUNT(Tweet.id).alias("ct")).where(Tweet.user == User.id)
        query = User.update(username=(User.username + "-x")).returning(
            subq.alias("ct"), User.username
        )
        result = query.execute()
        self.assertEqual(
            sorted([(r.ct, r.username) for r in result]),
        ...
        subq = (
            User.select(User.id, fn.COUNT(Tweet.id).alias("ct"))
            .join(Tweet, JOIN.LEFT_OUTER)
            .group_by(User.id)
        )
        query = (
            User.update(username=User.username + subq.c.ct)
            .from_(subq)
    ...
    def test_returning_integration(self):
        query = (
            User.insert_many(
                [("huey",), ("mickey",), ("zaizee",)], fields=[User.username]
            )
        ...
        result = query.execute()
        self.assertEqual(
            [(r.id, r.username) for r in result],
        ...
        result = query.execute()
        self.assertEqual(
            sorted([(r.id, r.username) for r in result]), [(2, "mickey"), (3, "zaizee")]
        )
...
class TestUpdateIntegrationRegressions(ModelTestCase):
    ...
    def test_update_examples(self):
        # Do a simple update.
        res = (
            User.update(username=(User.username + "-cat"))
        ...
        self.assertEqual(
            [u.username for u in users.clone()], ["huey-cat", "mickey", "zaizee-cat"]
        )

        # Do an update using a subquery..
        subq = User.select(User.username).where(User.username == "mickey")
        res = (
            User.update(username=(User.username + "-dog"))
            .where(User.username.in_(subq))
            .execute()
        )
        self.assertEqual(
            [u.username for u in users.clone()],
        ...
        res = (
            Tweet.update(content=(Tweet.content + "-x"))
        ...
        subq = Tweet.select(fn.COUNT(Tweet.id).cast("text")).where(
            Tweet.user == User.id
        )
    ...
    def test_update_examples_2(self):
        SA = Sample.alias()
        subq = SA.select(SA.value).where(SA.value.in_([1.0, 3.0]))
        res = (
            Sample.update(counter=(Sample.counter + Sample.value.cast("int")))
            .where(Sample.value.in_(subq))
            .execute()
        )

        query = Sample.select(Sample.counter, Sample.value).order_by(Sample.id).tuples()
        self.assertEqual(list(query.clone()), [(0, 0.0), (2, 1.0), (2, 2.0), (6, 3.0)])

        subq = SA.select(SA.counter - SA.value.cast("int")).where(
            SA.value == Sample.value
        )
        res = Sample.update(counter=subq).where(Sample.value.in_([1.0, 3.0])).execute()
        self.assertEqual(list(query.clone()), [(0, 0.0), (1, 1.0), (2, 2.0), (3, 3.0)])
...
class BaseVersionedModel(TestModel):
    ...
    def save_optimistic(self):
        ...
        query = ModelClass.update(**field_data).where(
            (ModelClass.version == current_version) & (ModelClass.id == self.id)
        )
        if query.execute() == 0:
...
class TestZeroTimestamp(ModelTestCase):
    ...
    def test_zero_timestamp(self):
        ...
        self.assertEqual(t1_db.timestamp, datetime.datetime(1970, 1, 1, 0, 0, 1))
...
class TestJoinSubqueryAggregateViaLeftOuter(ModelTestCase):
    ...
    def test_join_subquery_aggregate_left_outer(self):
        with self.database.atomic():
            ...
            score_list = ((10, 20, 30), (), (100, 110, 100), (50, 50))
        ...
        subq = (
            Game.select(
                Game.player,
                fn.SUM(Score.points).alias("ptotal"),
        ...
        query = (
            Player.select(Player, subq.c.ptotal, subq.c.pavg)
...
class Sku(TestModel):
    ...
    class Meta:
        constraints = [
            SQL("FOREIGN KEY (product_id, color) REFERENCES " "product(id, color)")
        ]

    @hybrid_property
    def product(self):
        if not hasattr(self, "_product"):
            self._product = Product.get(
                (Product.id == self.product_id) & (Product.color == self.color)
            )
...
class TestFKCompositePK(ModelTestCase):
    ...
    def test_fk_composite_pk_regression(self):
        Product.insert_many(
            [(1, "red"), (1, "blue"), (2, "red"), (2, "green"), (3, "white")]
        ).execute()
        Sku.insert_many(
            [
                ("1-red", 1, "red"),
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual(rows, [("1", "red", "1-red"), ("2", "red", "2-red")])

        query = (
            Sku.select(Sku, Product)
        ...
        with self.assertQueryCount(1):
            rows = [
                (s.upc, s.product_id, s.color, s.product.id, s.product.color)
                for s in query
            ]
            self.assertEqual(
                rows,
...
class TestReselectModelRegression(ModelTestCase):
    ...
    def test_reselect_model_regression(self):
        ...
        self.assertEqual(
            list(query.tuples()), [(u3.id, "u3",), (u2.id, "u2",), (u1.id, "u1",)]
        )
...
class TestJoinCorrelatedSubquery(ModelTestCase):
    ...
    def test_join_correlated_subquery(self):
        ...
        UA = User.alias()
        subq = UA.select(UA.username).where(UA.username.in_(("u0", "u2")))

        query = (
            Tweet.select(Tweet, User)
            .join(User, on=((Tweet.user == User.id) & (User.username.in_(subq))))
            .order_by(Tweet.id)
        )
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual(
                data, [("u0-0", "u0"), ("u2-0", "u2"), ("u2-1", "u2"), ("u2-2", "u2")]
            )
...
class TestMultiFKJoinRegression(ModelTestCase):
    ...
    def test_multi_fk_join_regression(self):
        ...
        Change = RU.alias()
        query = (
            Recipe.select(Recipe, RU, Change)
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual(
                data, [("r11", "u1", "u1"), ("r12", "u1", "u2"), ("r21", "u2", "u1")]
            )
...
class TestViewFieldMapping(ModelTestCase):
    ...
    def test_view_field_mapping(self):
        ...
        self.execute(
            "create view user_testview_fm as " "select id, username from users"
        )
        ...
        self.assertEqual(
            [(v.id, v.username) for v in View.select()], [(user.id, "huey")]
        )
...
class TestLikeColumnValue(ModelTestCase):
    ...
    def test_like_column_value(self):
        ...
        for user, tweets in data:
            Tweet.insert_many(
                [(user, tweet) for tweet in tweets], fields=[Tweet.user, Tweet.content]
            ).execute()
        ...
        for expr in expressions:
            query = Tweet.select(Tweet, User).join(User).where(expr).order_by(Tweet.id)

            self.assertEqual(
                [(t.user.username, t.content) for t in query],
                [("u1", "i am u1"), ("u1", "u1 is my name"), ("u3", "hey u3 is me")],
            )
...
class TestModelFilterJoinOrdering(ModelTestCase):
    ...
    def setUp(self):
        ...
        with self.database.atomic():
            ...
            p11, p12, p21 = [
                Page.create(site=s, title=t)
                for s, t in ((s1, "p1-1"), (s1, "p1-2"), (s2, "p2-1"))
            ]
            items = (
                (p11, "s1p1i1"),
                (p11, "s1p1i2"),
    ...
    def test_model_filter_join_ordering(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'WHERE ("t3"."url" = ?) ORDER BY "t1"."content"'
            ),
            ["s1"],
        )
        ...
        def assertQ(q):
            with self.assertQueryCount(1):
                self.assertEqual(
                    [pi.content for pi in q], ["s1p1i1", "s1p1i2", "s1p1i3", "s1p2i1"]
                )

        assertQ(q)

        sid = Site.get(Site.url == "s1").id
        q = PageItem.filter(page__site__url="s1", page__site__id=sid).order_by(
            PageItem.content
        )
        assertQ(q)

        q = (
            PageItem.filter(page__site__id=sid)
...
class TestCompositePKwithFK(ModelTestCase):
    ...
    def test_composite_pk_with_fk(self):
        ...
        query = CPKFK.select(CPKFK.key, CPK).join(CPK).order_by(CPKFK.key, CPK.name)
        with self.assertQueryCount(1):
            self.assertEqual(
                [(r.key, r.cpk.name) for r in query],
                [("k1", "c1"), ("k2", "c1"), ("k3", "c2")],
            )
...
class TestBulkCreateWithFK(ModelTestCase):
    ...
    def test_bulk_create_with_fk(self):
        ...
        with self.assertQueryCount(1):
            BCTweet.bulk_create(
                [BCTweet(user="u1", content="t%s" % i) for i in range(4)]
            )
...
class TestFKMigrationRegression(ModelTestCase):
    ...
    def test_fk_migration(self):
        ...
        migrate(migrator.drop_column(FKMB._meta.table_name, FKMB.fkma.column_name))

        migrate(
            migrator.add_column(FKMB._meta.table_name, FKMB.fkma.column_name, FKMB.fkma)
        )
