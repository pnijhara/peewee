...
class TestForeignKeyToNonPrimaryKey(ModelTestCase):
    ...
    def setUp(self):
        ...
        for barcode in ["101", "102"]:
            ...
            for i in range(2):
                PackageItem.create(package=barcode, title="%s-%s" % (barcode, i))
...
class TestMultipleForeignKey(ModelTestCase):
    ...
    def test_multi_join(self):
        ...
        query = (
            Computer.select(
                Computer, HDD, Memory, Processor, HDDMf, MemoryMf, ProcessorMf
            )
            .join(HDD, on=(Computer.hard_drive_id == HDD.id).alias("hard_drive"))
            .join(HDDMf, JOIN.LEFT_OUTER, on=(HDD.manufacturer_id == HDDMf.id))
            .switch(Computer)
            .join(Memory, on=(Computer.memory_id == Memory.id).alias("memory"))
            .join(MemoryMf, JOIN.LEFT_OUTER, on=(Memory.manufacturer_id == MemoryMf.id))
            .switch(Computer)
            .join(
                Processor, on=(Computer.processor_id == Processor.id).alias("processor")
            )
            .join(
                ProcessorMf,
                JOIN.LEFT_OUTER,
        ...
        with self.assertQueryCount(1):
            ...
            for computer in query:
                components = [computer.hard_drive, computer.memory, computer.processor]
            ...
            self.assertEqual(
                manufacturers, [None, "Kingston", "Intel", None, "Kingston", "AMD",]
            )
...
class TestCompositePrimaryKey(ModelTestCase):
    ...
    def test_get_set_id(self):
        tpt = (
            TagPostThrough.select()
            .join(Tag)
            .switch(TagPostThrough)
            .join(Post)
            .order_by(Tag.tag, Post.title)
        ).get()
        # Sanity check.
    ...
    def test_composite_key_model(self):
        ...
        values = [("a", 1, 1.0), ("a", 2, 2.0), ("b", 1, 1.0), ("b", 2, 2.0)]
        c1, c2, c3, c4 = [CKM.create(f1=f1, f2=f2, f3=f3) for f1, f2, f3 in values]
        ...
        c4.save()
        self.assertRaises(
            CKM.DoesNotExist, lambda: CKM.get((CKM.f1 == "c") & (CKM.f2 == 2))
        )

    def test_count_composite_key(self):
        CKM = CompositeKeyModel
        values = [("a", 1, 1.0), ("a", 2, 2.0), ("b", 1, 1.0), ("b", 2, 1.0)]
        for f1, f2, f3 in values:
            CKM.create(f1=f1, f2=f2, f3=f3)

        self.assertEqual(CKM.select().count(), 4)
        self.assertTrue(CKM.select().where((CKM.f1 == "a") & (CKM.f2 == 1)).exists())
        self.assertFalse(CKM.select().where((CKM.f1 == "a") & (CKM.f2 == 3)).exists())
    ...
    def test_composite_key_inheritance(self):
        ...
        self.assertEqual(Person._meta.primary_key.field_names, ("first", "last"))

        class Employee(Person):
            title = TextField()

        self.assertTrue(isinstance(Employee._meta.primary_key, CompositeKey))
        self.assertEqual(Employee._meta.primary_key.field_names, ("first", "last"))
        sql = (
