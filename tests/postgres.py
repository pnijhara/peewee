...
class TestHStoreField(ModelTestCase):
    database = db_loader(
        "postgres", db_class=PostgresqlExtDatabase, register_hstore=True
    )
    ...
    def query(self, *cols):
        return HStoreModel.select(HStoreModel.name, *cols).order_by(HStoreModel.id)

    def test_hstore_selecting(self):
        query = self.query(D.keys().alias("keys"))
        self.assertEqual(
            [(x.name, sorted(x.keys)) for x in query],
            [("t1", ["k1", "k2"]), ("t2", ["k2", "k3"])],
        )

        query = self.query(D.values().alias("vals"))
        self.assertEqual(
            [(x.name, sorted(x.vals)) for x in query],
            [("t1", ["v1", "v2"]), ("t2", ["v2", "v3"])],
        )

        query = self.query(D.items().alias("mtx"))
        self.assertEqual(
            [(x.name, sorted(x.mtx)) for x in query],
            [
                ("t1", [["k1", "v1"], ["k2", "v2"]]),
                ("t2", [["k2", "v2"], ["k3", "v3"]]),
            ],
        ...
        self.assertEqual(
            [(x.name, x.kz) for x in query],
            [("t1", {"k2": "v2"}), ("t2", {"k2": "v2", "k3": "v3"})],
        )

        query = self.query(D.slice("k4").alias("kz"))
        self.assertEqual([(x.name, x.kz) for x in query], [("t1", {}), ("t2", {})])

        query = self.query(D.exists("k3").alias("ke"))
        self.assertEqual([(x.name, x.ke) for x in query], [("t1", False), ("t2", True)])

        query = self.query(D.defined("k3").alias("ke"))
        self.assertEqual([(x.name, x.ke) for x in query], [("t1", False), ("t2", True)])

        query = self.query(D["k1"].alias("k1"))
        self.assertEqual([(x.name, x.k1) for x in query], [("t1", "v1"), ("t2", None)])
    ...
    def test_hstore_update(self):
        rc = (
            HStoreModel.update(data=D.update(k4="v4"))
            .where(HStoreModel.name == "t1")
            .execute()
        )
        self.assertTrue(rc > 0)

        self.assertEqual(self.by_name("t1"), {"k1": "v1", "k2": "v2", "k4": "v4"})

        rc = (
            HStoreModel.update(data=D.update(k5="v5", k6="v6"))
        ...
        self.assertEqual(
            self.by_name("t2"), {"k2": "v2", "k3": "v3", "k5": "v5", "k6": "v6"}
        )
        ...
        (
            HStoreModel.update(data=D.delete("k4"))
            .where(HStoreModel.name == "t1")
            .execute()
        )
        ...
        self.assertEqual(
            [x.data for x in self.query(D)],
            [{"k1": "v1", "k2": "vxxx"}, {"k2": "vxxx", "k3": "v3", "k6": "v6"}],
        )

        HStoreModel.update(data=D.delete("k1", "k2")).execute()
        self.assertEqual(
            [x.data for x in self.query(D)], [{}, {"k3": "v3", "k6": "v6"}]
        )
...
class TestArrayField(ModelTestCase):
    ...
    def create_sample(self):
        return ArrayModel.create(
            tags=["alpha", "beta", "gamma", "delta"], ints=[[1, 2], [3, 4], [5, 6]]
        )
    ...
    def test_hashable_objectslice(self):
        ...
        n = ArrayModel.update(
            {ArrayModel.ints[0][0]: ArrayModel.ints[0][0] + 1}
        ).execute()
    ...
    def test_array_search(self):
        def assertAM(where, *instances):
            query = ArrayModel.select().where(where).order_by(ArrayModel.id)
...
class TestArrayFieldConvertValues(ModelTestCase):
    ...
    def test_value_conversion(self):
        ...
        ts = ArrayTSModel.get(ArrayTSModel.timestamps.contains(self.dt(4, 5, 6, 7)))
        self.assertEqual(ts.key, "k3")

        self.assertRaises(
            ArrayTSModel.DoesNotExist,
            ArrayTSModel.get,
    ...
    def test_get_with_array_values(self):
        ...
        query = ArrayTSModel.select().where(ArrayTSModel.timestamps == [self.dt(1)])
        a1_db = query.get()
        self.assertEqual(a1_db.id, a1.id)

        query = ArrayTSModel.select().where(
            ArrayTSModel.timestamps == [self.dt(2), self.dt(3)]
        )
        a2_db = query.get()
...
class TestArrayUUIDField(ModelTestCase):
    ...
    def test_array_of_uuids(self):
        ...
        a = UUIDList.create(key="a", id_list=[u1, u2, u3], id_list_native=[u1, u2, u3])
        b = UUIDList.create(key="b", id_list=[u2, u3, u4], id_list_native=[u2, u3, u4])
...
class TestTSVectorField(ModelTestCase):
    ...
    def setUp(self):
        ...
        for idx, message in enumerate(self.messages):
            FTSModel.create(
                title=str(idx), data=message, fts_data=fn.to_tsvector(message)
            )
    ...
    def test_sql(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'WHERE (to_tsvector("t1"."data") @@ to_tsquery(?))'
            ),
            ["foo bar"],
        )
...
class TestJsonField(BaseJsonFieldTestCase, ModelTestCase):
    ...
    def test_json_null(self):
        ...
        self.assertEqual([tj_db.data for tj_db in results], [None, {"k1": "v1"}])

        query = JsonModelNull.select().where(JsonModelNull.data.is_null(True))
    ...
    def test_integer_index_weirdness(self):
        ...
        def fails():
            with self.database.atomic():
                ...
                results = list(BJson.select().where(BJson.data.contains_any(2, 8, 12)))
...
class TestJsonFieldRegressions(ModelTestCase):
    ...
    def test_json_field_concat(self):
        ...
        obj = query.get()
        self.assertEqual(
            obj.data, {"k1": {"x2": "y2"}, "k2": "v2-x", "k3": "v3", "k4": "v4"}
        )
...
class TestIntervalField(ModelTestCase):
    ...
    def test_interval_field(self):
        ...
        events = [(e.name, e.duration) for e in Event.select().order_by(Event.duration)]
        self.assertEqual(
            events,
            [
...
class TestIndexedField(BaseTestCase):
    def test_indexed_field_ddl(self):
        ...
        indexes = [
            idx.query()[0] for idx in IndexedModel._schema._create_indexes(False)
        ]
        self.assertEqual(
            indexes,
            [
...
class TestIdentityField(ModelTestCase):
    ...
    def test_identity_field_always(self):
        ...
        self.assertEqual(list(query.tuples()), [(1, "d1"), (2, "d2"), (3, "d3")])
    ...
    def test_identity_field_by_default(self):
        ...
        self.assertEqual(
            list(query.tuples()), [(1, "d1"), (2, "d2"), (3, "d3"), (10, "d10")]
        )
...
class TestAutocommitIntegration(ModelTestCase):
    ...
    def test_autocommit_default(self):
        ...
        self.assertEqual(
            [(kx.key, kx.value) for kx in KX.select().order_by(KX.key)],
            [("k1", 1), ("k2", 2)],
        )
...
class TestPostgresLateralJoin(ModelTestCase):
    ...
    def test_lateral_top_n(self):
        ...
        subq = (
            Tweet.select(Tweet.content, Tweet.timestamp)
            .where(Tweet.user == User.id)
            .order_by(Tweet.timestamp.desc())
            .limit(2)
        )
        query = (
            User.select(User, subq.c.content)
            .join(subq, JOIN.LEFT_LATERAL)
            .order_by(subq.c.timestamp.desc(nulls="last"))
        )
        results = [(u.username, u.content) for u in query]
        self.assertEqual(
            results, [("a", "a10"), ("b", "b7"), ("b", "b4"), ("a", "a2"), ("c", None)]
        )

        query = (
            Tweet.select(User.username, subq.c.content)
            .from_(User)
            .join(subq, JOIN.LEFT_LATERAL)
        ...
        self.assertEqual(
            results, [("a", "a2"), ("a", "a10"), ("b", "b4"), ("b", "b7"), ("c", None)]
        )
