...
class TestSchemaMigration(ModelTestCase):
    ...
    def test_add_table_constraint(self):
        ...
        migrate(
            ...
            self.migrator.add_constraint("tag", "price_check", Check("price >= 0")),
        )
    ...
    def test_add_column_constraint(self):
        ...
        ff = FloatField(default=0.0, constraints=[Check("val < 1.0")])
        t1 = Tag.create(tag="t1")
        migrate(
        ...
        self.assertEqual(t1_db.val, 0.0)

        with self.database.atomic():
            self.assertRaises(
                IntegrityError, NewTag.create, tag="t2", misc="bar", val=2.0
            )
    ...
    def test_drop_column(self):
        ...
        self.assertEqual(data, [("charlie",), ("huey",),])
    ...
    def test_rename_column(self):
        ...
        query = NewPerson.select(
            NewPerson.first, NewPerson.last, NewPerson.dob
    ...
    def test_rename_gh380(self):
        ...
        query = NewPage.select(NewPage.title, NewPage.user).order_by(NewPage.title)
        self.assertEqual(
    ...
    def test_add_not_null(self):
        ...
        (
            Person.update(dob=datetime.date(2000, 1, 2))
        ...
        with self.database.transaction():
            with self.assertRaisesCtx((IntegrityError, OperationalError)):
                Person.create(first_name="Kirby", last_name="Snazebrauer", dob=None)
    ...
    def test_drop_not_null(self):
        ...
        query = Person.select().where(
            (Person.first_name >> None) & (Person.last_name >> None)
        )
    ...
    def test_modify_not_null_foreign_key(self):
        ...
        with self.database.transaction():
            with self.assertRaisesCtx((OperationalError, IntegrityError)):
                Page.create(name="fails", user=None)
    ...
    def test_rename_table(self):
        ...
        query = Tag_asdf.select().order_by(Tag_asdf.tag)
        self.assertEqual([t.tag for t in query], ["t1", "t2"])

        # Verify the old table is gone.
        with self.database.transaction():
            self.assertRaises(DatabaseError, Tag.create, tag="t3")
    ...
    def test_add_unique_column(self):
        ...
        with self.database.atomic():
            self.assertRaises(
                IntegrityError, NewTag.create, tag="t3", unique_field="u1"
            )
    ...
    def test_drop_index(self):
        ...
        migrate(self.migrator.drop_index("person", "person_first_name_last_name"))
    ...
    def test_multiple_operations(self):
        ...
        with self.database.transaction():
            self.assertRaises(
                IntegrityError, PersonNugg.create, last="Leifer", field_default="bazer"
            )
    ...
    def test_drop_foreign_key(self):
        ...
        self.assertEqual(sorted(column.name for column in columns), ["id", "name"])
    ...
    def test_rename_foreign_key(self):
        ...
        columns = self.database.get_columns("page")
        self.assertEqual(
            sorted(column.name for column in columns), ["huey_id", "id", "name"]
        )
    ...
    def test_rename_unique_foreign_key(self):
        ...
        self.assertEqual(
            sorted(column.name for column in columns), ["huey_id", "id", "updated_at"]
        )
    ...
    def test_valid_column_required(self):
        ...
        self.assertRaises(
            ValueError, migrate, self.migrator.rename_column("page", "xx", "yy")
        )
    ...
    def test_add_column_indexed_table(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                ("index_model_first_name_last_name", ["first_name", "last_name"]),
            ],
        )
    ...
    def test_rename_column_to_table_name(self):
        ...
        indexes = lambda: sorted(
            (idx.name, idx.columns) for idx in db.get_indexes("page")
        )
    ...
    def test_add_fk_with_constraints(self):
        ...
        field = ForeignKeyField(
            Category, Category.id, backref="children", null=True, on_delete="SET NULL"
        )
        migrate(self.migrator.add_column(Category._meta.table_name, "parent_id", field))
        queries = [x.msg for x in self.history]
        self.assertEqual(
            queries,
            [
    ...
    def test_index_preservation(self):
        self.reset_sql_history()
        migrate(self.migrator.rename_column("index_model", "first_name", "first"))
        ...
        self.assertEqual(
            ...
            [
                ...
                ('PRAGMA "main".index_info("index_model_first_name_last_name")', None),
                # Get foreign keys.
...
class TestSqliteColumnNameRegression(ModelTestCase):
    ...
    def test_sqlite_column_name_regression(self):
        ...
        migrate(
            migrator.add_column("bad_names", "new_data", new_data),
        ...
        self.assertEqual(
            column_names, ["id", "primary_data", "foreign_data", "new_data"]
        )

        BNT = Table(
            "bad_names", ("id", "primary_data", "foreign_data", "new_data")
        ).bind(self.database)
        self.assertEqual(
            [row for row in BNT.select()],
            [{"id": 1, "primary_data": "pd", "foreign_data": "fd", "new_data": "foo"}],
        )
