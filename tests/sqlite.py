...
class ContentPost(FTSModel, Post):
    class Meta:
        options = {"content": Post, "tokenize": "porter"}
...
class TestTableFunction(BaseTestCase):
    ...
    def test_split(self):
        ...
        curs = self.execute(
            "select part from str_split(?) order by part " "limit 3",
            ("well hello huey and zaizee",),
        )
        self.assertEqual([row for row, in curs.fetchall()], ["and", "hello", "huey"])
    ...
    def test_split_tbl(self):
        ...
        self.execute(
            ...
            ("huey secret post", "mickey message", "zaizee diary"),
        )
    ...
    def test_series_tbl(self):
        ...
        results = curs.fetchall()
        self.assertEqual(results, [(1, 1), (1, 2), (1, 3), (2, 2), (2, 3), (2, 4)])
    ...
    def test_regex(self):
        ...
        assertResults(
            "[0-9]+", "foo 123 45 bar 678 nuggie 9.0", ["123", "45", "678", "9", "0"]
        )
        assertResults(
            "[\w]+@[\w]+\.[\w]{2,3}",
            (
        ...
        assertResults("[a-z]+", "123.pDDFeewXee", ["p", "eew", "ee"])
        assertResults("[0-9]+", "hello", [])
    ...
    def test_regex_tbl(self):
        ...
        self.execute("insert into posts (msg) values (?), (?), (?), (?)", messages)
        cur = self.execute(
            "select posts.id, regex_search.rowid, regex_search.match "
    ...
    def test_error_init(self):
        ...
        self.assertRaises(
            sqlite3.OperationalError, self.execute, "SELECT * FROM broken_init(1, 10)"
        )
        self.assertRaises(
            sqlite3.OperationalError, self.execute, "SELECT * FROM broken_init(0, 1)"
        )
    ...
    def test_error_iterate(self):
        ...
        self.assertRaises(
            sqlite3.OperationalError, self.execute, "SELECT * FROM broken_iterate(0, 1)"
        )
...
class TestJSONField(ModelTestCase):
    ...
    def test_create_read_update(self):
        ...
        for value in test_values:
            nrows = KeyData.update(data=value).where(KeyData.key == "kx").execute()
    ...
    def test_json_to_json(self):
        ...
        subq = KeyData.select(KeyData.data).where(KeyData.key == "k1")
...
class TestJSONFieldFunctions(ModelTestCase):
    ...
    def test_json_group_functions(self):
        with self.database.atomic():
            ...
            for i in range(10):
                ...
                KeyData.create(
                    key="k%s" % i, data={"v": i, "v%s" % i: {"items": list(range(i))}}
                )

        jga_key = fn.json_group_array(KeyData.key)
        query = (
            KeyData.select(jga_key).where(KeyData.data["v"] < 4).order_by(KeyData.key)
        )
        self.assertEqual(json.loads(query.scalar()), ["k0", "k1", "k2", "k3"])

        # Can specify json.loads as the converter for the function.
        query = (
            KeyData.select(jga_key.python_value(json.loads))
        ...
        query = KeyData.select(jga_id).where(KeyData.data["v"] < 4).order_by(KeyData.id)
        self.assertEqual(json.loads(query.scalar()), [1, 2, 3, 4])

        query = (
            KeyData.select(jga_id.python_value(json.loads))
            .where(KeyData.data["v"] > 6)
            .order_by(KeyData.id)
        )
        ...
        res = KeyData.select(jgo_key).where(KeyData.data["v"] < 4).scalar()
        self.assertEqual(json.loads(res), {"k0": 0, "k1": 1, "k2": 2, "k3": 3})

        query = KeyData.select(jgo_key.python_value(json.loads)).where(
            KeyData.data["v"] < 4
        )
    ...
    def test_set(self):
        query = KeyData.update({KeyData.data: KeyData.data["k1"].set("v1-x")}).where(
            KeyData.data["k1"] == "v1"
        )
    ...
    def test_set_json(self):
        ...
        query = KeyData.update({KeyData.data: set_json}).where(
            KeyData.data["x1"]["y1"] == "z1"
        )
        self.assertEqual(query.execute(), 2)
    ...
    def test_update(self):
        ...
        query = KeyData.update({KeyData.data: merged}).where(
            KeyData.data["x1"]["y1"] == "z1"
        )
        self.assertEqual(query.execute(), 2)
        self.assertRows((KeyData.data["x1"]["y1"] == "z1-x"), ["a", "d"])
    ...
    def test_update_nested(self):
        ...
        query = KeyData.update(data=merged).where(KeyData.data["x1"]["y1"] == "z1")
        self.assertEqual(query.execute(), 2)
        self.assertRows((KeyData.data["x1"]["y1"] == "z1-x"), ["a", "d"])
    ...
    def test_remove(self):
        query = KeyData.update(data=KeyData.data["k1"].remove()).where(
            KeyData.data["k1"] == "v1"
        )
        self.assertEqual(query.execute(), 2)
        ...
        nrows = (
            KeyData.update(data=KeyData.data["l2"][1][1].remove())
            .where(KeyData.key == "e")
            .execute()
        )
    ...
    def test_children(self):
        ...
        query = (
            KeyData.select(KeyData.key, children.c.fullkey.alias("fullkey"))
        ...
        self.assertEqual(
            accum,
            [("a", "$.x1"), ("b", "$.x2"), ("d", "$.x1"), ("e", "$.l1"), ("e", "$.l2")],
        )

    def test_tree(self):
        tree = KeyData.data.tree().alias("tree")
        query = (
            KeyData.select(tree.c.fullkey.alias("fullkey"))
            .from_(KeyData, tree)
            .where(KeyData.key == "d")
            .order_by(SQL("1"))
            .tuples()
        ...
        self.assertEqual(
            [fullkey for fullkey, in query], ["$", "$.x1", "$.x1.y1", "$.x1.y2"]
        )
...
class TestSqliteExtensions(BaseTestCase):
    ...
    def test_autoincrement_field(self):
        ...
        self.assertSQL(
            AutoIncrement._schema._create_table(),
            (
...
class BaseFTSTestCase(object):
    ...
    def assertMessages(self, query, indexes):
        self.assertEqual(
            [obj.message for obj in query], [self.messages[idx] for idx in indexes]
        )


class TestFullTextSearch(BaseFTSTestCase, ModelTestCase):
    database = database
    requires = [Post, ContentPost, ContentPostMessage, Document, MultiColumn]
    ...
    def test_fts_manual(self):
        messages = [Document.create(message=message) for message in self.messages]
        query = (
            Document.select().where(Document.match("believe")).order_by(Document.docid)
        )
        ...
        self.assertEqual(
            [(row.message, row.score) for row in query],
        ...
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query],
            [(self.messages[4], -0.45), (self.messages[2], -0.36)],
        )
        ...
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query],
            [(self.messages[3], -0.49), (self.messages[0], -0.35)],
        )

        query = Document.search_bm25("god faith", with_score=True)
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query], [(self.messages[1], -0.92)]
        )

        query = Document.search_bm25('"it is"', with_score=True)
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query],
            [(self.messages[2], -0.36), (self.messages[3], -0.36)],
        )
    ...
    def test_fts_delete_row(self):
        ...
        ContentPost.rebuild()
        query = (
            ContentPost.select(ContentPost, ContentPost.rank().alias("score"))
            .where(ContentPost.match("believe"))
            .order_by(ContentPost.docid)
        )
        self.assertMessages(query, [0, 3])

        query = ContentPost.select(ContentPost.docid).order_by(ContentPost.docid)
        ...
        documents = [Document.create(message=message) for message in self.messages]
    ...
    def test_fts_multi_column(self):
        ...
        assertResults("bbbbb", [(2, -1.5), (1, -0.5)])  # 1/2 + 1/1  # 1/2
        ...
        assertResults("zzzzz", [(1, -0.67), (2, -0.33)])
    ...
    def test_bm25(self):
        def assertResults(term, expected):
            ...
            self.assertEqual([(mc.c4, round(mc.score, 2)) for mc in query], expected)
        ...
        self.assertEqual(
            [(mc.c4, round(mc.score, 2)) for mc in query], [(5, -0.39), (3, -0.3)]
        )

        # Use helpers.
        query = (
            MultiColumn.select(MultiColumn.c4, MultiColumn.bm25(1.0).alias("score"))
            .where(MultiColumn.match("aaaaa"))
            .order_by(SQL("score"))
        )
        self.assertEqual(
            [(mc.c4, round(mc.score, 2)) for mc in query], [(5, -0.39), (1, -0.3)]
        )
    ...
    def test_bm25_alt_corpus(self):
        ...
        self.assertEqual([round(d.score, 2) for d in query], [-0.0] * 5)
    ...
    def _test_fts_auto(self, ModelClass):
        ...
        self.assertEqual(
            [(x.message, x.score) for x in pq],
            [(self.messages[4], -2.0 / 3), (self.messages[2], -1.0 / 3),],
        )

        pq = (
            ModelClass.select(ModelClass.rank())
            .where(ModelClass.match("faithful"))
            .tuples()
        )
        self.assertEqual([x[0] for x in pq], [-0.2] * 5)

        pq = ModelClass.search("faithful", with_score=True).dicts()
        self.assertEqual([x["score"] for x in pq], [-0.2] * 5)
    ...
    def test_weighting(self):
        ...
        assertResults(
            ...
            "bbbbb",
            None,
            [(2, -1.5), (1, -0.5),],  # 1/2 + 1/1  # 1/2
        )
        assertResults(
            MultiColumn.search,
            "bbbbb",
            [1.0, 5.0, 0.0],
            [(2, -5.5), (1, -0.5),],  # 1/2 + (5 * 1/1)  # 1/2 + (5 * 0)
        )
        assertResults(
            MultiColumn.search,
            "bbbbb",
            [1.0, 0.5, 0.0],
            [(2, -1.0), (1, -0.5),],  # 1/2 + (.5 * 1/1)  # 1/2 + (.5 * 0)
        )
        assertResults(
            MultiColumn.search,
            "bbbbb",
            [1.0, -1.0, 0.0],
            [(1, -0.5), (2, 0.5),],  # 1/2 + (-1 * 0)  # 1/2 + (-1 * 1/1)
        )

        # BM25
        assertResults(MultiColumn.search_bm25, "bbbbb", None, [(2, -0.85), (1, -0.0)])
        assertResults(
            MultiColumn.search_bm25, "bbbbb", [1.0, 5.0, 0.0], [(2, -4.24), (1, -0.0)]
        )
        assertResults(
            MultiColumn.search_bm25, "bbbbb", [1.0, 0.5, 0.0], [(2, -0.42), (1, -0.0)]
        )
        assertResults(
            MultiColumn.search_bm25, "bbbbb", [1.0, -1.0, 0.0], [(1, -0.0), (2, 0.85)]
        )
    ...
    def test_fts_match_single_column(self):
        ...
        def assertSearch(field, value, expected):
            query = (
                MultiColumn.select().where(field.match(value)).order_by(MultiColumn.c1)
            )
    ...
    def test_fts_score_single_column(self):
        ...
        def assertQueryScore(field, search_term, expected, *weights):
            ...
            query = (
                MultiColumn.select(MultiColumn, rank.alias("score"))
        ...
        assertQueryScore(MultiColumn.c2, "bbbb", [("m1", -0.0), ("m3", -0.0)])
        assertQueryScore(MultiColumn.c2, "eeee", [("m2", -0.51)])
        assertQueryScore(MultiColumn.c3, "aaaa", [("m3", -0.62)])

        assertQueryScore(MultiColumn.c1, "aaaa", [("m1", -1.02)], 2.0, 0.0, 0.0)
        assertQueryScore(
            MultiColumn.c2, "bbbb", [("m1", -0.0), ("m3", -0.0)], 0.0, 1.0, 0.0
        )
        assertQueryScore(MultiColumn.c2, "eeee", [("m2", -1.02)], 0.0, 2.0, 0.0)
        assertQueryScore(MultiColumn.c3, "aaaa", [("m3", -0.31)], 0.0, 1.0, 0.5)
    ...
    def test_match_column_queries(self):
        ...
        def assertQ(expr, idxscore):
            q = (
                MC.select(MC, MC.bm25().alias("score"))
                .where(expr)
        ...
        assertQ(MC.match("beta"), [(1, -0.85), (0, -0.0), (3, -0.0)])
        ...
        assertQ(MC.match('"delta"'), [(3, -0.85), (2, -0.0)])
        assertQ(MC.match("c1:delta OR c2:delta"), [(3, -0.85), (2, -0.0)])
        assertQ(MC.match('"^delta"'), [(3, -1.69)])

        assertQ(MC.match("(delta AND c2:apple) OR c1:alpha"), [(3, -0.85), (0, -0.85)])
        assertQ(
            MC.match("(c2:delta AND c2:apple) OR c1:alpha"), [(0, -0.85), (3, -0.0)]
        )
        assertQ(MC.match("c2:delta c2:apple OR c1:alpha"), [(0, -0.85), (3, -0.0)])
        assertQ(
            MC.match("(c2:delta AND c2:apple) OR beta"),
            [(1, -0.85), (3, -0.0), (0, -0.0)],
        )
        assertQ(MC.match("c2:delta AND (c2:apple OR c1:alpha)"), [(3, -0.0)])

        # c2 apple (0,1,3) OR (...irrelevant...).
        assertQ(
            MC.match("c2:apple OR c1:alpha NOT delta"),
            [(0, -0.85), (1, -0.0), (3, -0.0)],
        )
        assertQ(
            MC.match("c2:apple OR (c1:alpha NOT c2:delta)"),
            [(0, -0.85), (1, -0.0), (3, -0.0)],
        )
        # c2 apple OR c1 alpha (0, 1, 3) AND NOT delta (2, 3) -> (0, 1).
        assertQ(MC.match("(c2:apple OR c1:alpha) NOT delta"), [(0, -0.85), (1, -0.0)])
...
class TestFullTextSearchCython(TestFullTextSearch):
    ...
    def test_bm25f(self):
        ...
        # No column specified, use the first text field.
        query = MultiColumn.search_bm25f("aaaaa OR fffff", [1.0, 3.0, 0, 0], 1)
        self.assertEqual(
            [(mc.c4, round(mc.score, 2)) for mc in query],
            [(1, -14.18), (5, -12.01), (3, -11.48)],
        )
    ...
    def test_lucene(self):
        ...
        assertResults("things", [(-0.166, "Faith has"), (-0.137, "Be faithful")])
...
class TestFTS5(BaseFTSTestCase, ModelTestCase):
    ...
    def assertResults(self, query, expected, scores=False, alias="score"):
        if scores:
            results = [(obj.title, round(getattr(obj, alias), 7)) for obj in query]
        else:
    ...
    def test_search(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'WHERE ("fts5_test" MATCH ?) ORDER BY rank'
            ),
            ["bb"],
        )
        ...
        self.assertResults(
            FTS5Test.search("baze OR dd"), ["baze cc dd", "bar bb cc", "nug aa dd"]
        )

    @requires_models(FTS5Document)
    def test_fts_manual(self):
        messages = [FTS5Document.create(message=message) for message in self.messages]
        query = (
            FTS5Document.select()
        ...
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query],
            [(self.messages[4], -0.45), (self.messages[2], -0.37)],
        )
        ...
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query],
            [(self.messages[3], -0.49), (self.messages[0], -0.36)],
        )

        query = FTS5Document.search_bm25("god faith", with_score=True)
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query], [(self.messages[1], -0.93)]
        )

        query = FTS5Document.search_bm25('"it is"', with_score=True)
        self.assertEqual(
            [(d.message, round(d.score, 2)) for d in query],
            [(self.messages[2], -0.37), (self.messages[3], -0.37)],
        )
    ...
    def test_match_column_queries(self):
        ...
        def assertQ(expr, idxscore):
            q = (
                FT.select(FT, FT.bm25().alias("score"))
                .where(expr)
                .order_by(SQL("score"), FT.misc.cast("int"))
            )
            self.assertEqual([(int(r.misc), round(r.score, 2)) for r in q], idxscore)
        ...
        assertQ(
            FT.match("(delta AND data:apple) OR title:alpha"), [(3, -2.09), (0, -2.02)]
        )
        assertQ(
            FT.match("(data:delta AND data:apple) OR title:alpha"),
            [(0, -2.02), (3, -1.76)],
        )
        assertQ(
            FT.match("data:delta data:apple OR title:alpha"), [(0, -2.02), (3, -1.76)]
        )
        assertQ(
            FT.match("(data:delta AND data:apple) OR beta"),
            [(3, -2.33), (1, -0.74), (0, -0.57)],
        )
        assertQ(FT.match("data:delta AND (data:apple OR title:alpha)"), [(3, -1.76)])

        # data apple (0,1,3) OR (...irrelevant...).
        assertQ(
            FT.match("data:apple OR title:alpha NOT delta"),
            [(0, -2.58), (1, -0.58), (3, -0.57)],
        )
        assertQ(
            FT.match("data:apple OR (title:alpha NOT data:delta)"),
            [(0, -2.58), (1, -0.58), (3, -0.57)],
        )
        # data apple OR title alpha (0, 1, 3) AND NOT delta (2, 3) -> (0, 1).
        assertQ(
            FT.match("(data:apple OR title:alpha) NOT delta"), [(0, -2.58), (1, -0.58)]
        )
...
class TestMurmurHash(ModelTestCase):
    database = SqliteExtDatabase(
        ":memory:", c_extensions=CYTHON_EXTENSION, hash_functions=True
    )
...
class TestUserDefinedCallbacks(ModelTestCase):
    ...
    def test_custom_agg(self):
        ...
        self.assertEqual(q_data, [(1, 4.7, 4.7), (2, 3.4, 3.4), (3, 2.6, 2.6)])
        ...
        self.assertEqual(q_data, [(1, "5.23", 4.7), (2, "3.4", 3.4), (3, "2.6", 2.6)])
    ...
    def test_custom_collation(self):
        ...
        pq = Post.select().order_by(
            NodeList((Post.message, SQL("collate collate_reverse")))
        )
    ...
    def test_collation_decorator(self):
        ...
        self.assertEqual([p.message for p in pq], ["aaa", "Aab", "Bba", "BbB", "ccc"])
    ...
    def test_custom_function(self):
        ...
        self.assertEqual([x[0] for x in sq], ["This Is A Test", "Another Test",])

    def test_function_decorator(self):
        [Post.create(message=m) for m in ["testing", "chatting  ", "  foo"]]
        pq = Post.select(fn.rstrip(Post.message, "ing")).order_by(Post.id)
        self.assertEqual([x[0] for x in pq.tuples()], ["test", "chatting  ", "  foo"])

        pq = Post.select(fn.rstrip(Post.message, " ")).order_by(Post.id)
        self.assertEqual([x[0] for x in pq.tuples()], ["testing", "chatting", "  foo"])
...
class TestTransitiveClosure(BaseTestCase):
    def test_model_factory(self):
        ...
        self.assertEqual(
            Closure._meta.options,
            {"idcolumn": "id", "parentcolumn": "parent_id", "tablename": "category",},
        )
        ...
        self.assertEqual(
            Closure._meta.options,
            {"idcolumn": "pk", "parentcolumn": "ref_id", "tablename": "alt",},
        )
...
@skip_unless(
    CLOSURE_EXTENSION and os.path.exists(CLOSURE_EXTENSION),
...
class TestTransitiveClosureIntegration(BaseTestCase):
    ...
    def assertNodes(self, query, *expected):
        self.assertEqual(set([category.name for category in query]), set(expected))
    ...
    def test_descendants(self):
        ...
        self.assertNodes(Closure.descendants(books, 1), "fiction", "non-fiction")
        self.assertNodes(
        ...
        self.assertNodes(Closure.descendants(books, 3), "hard scifi", "dystopian")
        ...
        self.assertNodes(
            Closure.descendants(fiction, 1), "scifi", "westerns", "classics"
        )
        self.assertNodes(Closure.descendants(fiction, 2), "hard scifi", "dystopian")

        self.assertNodes(
            Closure.descendants(Category.g("scifi")), "hard scifi", "dystopian"
        )
        self.assertNodes(
    ...
    def test_ancestors(self):
        ...
        self.assertNodes(Closure.ancestors(hard_scifi), "scifi", "fiction", "books")
        self.assertNodes(
        ...
        self.assertNodes(
            Closure.ancestors(non_fiction, include_node=True), "non-fiction", "books"
        )
        self.assertNodes(Closure.ancestors(non_fiction, 1), "books")

        books = Category.g("books")
        self.assertNodes(Closure.ancestors(books, include_node=True), "books")
    ...
    def test_siblings(self):
        ...
        self.assertNodes(Closure.siblings(Category.g("hard scifi")), "dystopian")
        self.assertNodes(
            Closure.siblings(Category.g("hard scifi"), include_node=True),
            "hard scifi",
            "dystopian",
        )
        self.assertNodes(Closure.siblings(Category.g("classics")), "scifi", "westerns")
        self.assertNodes(
        ...
        self.assertNodes(Closure.siblings(Category.g("fiction")), "non-fiction")
    ...
    def test_tree_changes(self):
        ...
        query = (
            Category.select(Category.name, Closure.depth)
    ...
    def test_id_not_overwritten(self):
        ...
        self.assertEqual(
            sorted([(n.id, n.name) for n in query]), [(c1.id, "c1"), (c2.id, "c2")]
        )
...
@skip_unless(
    LSM_EXTENSION and os.path.exists(LSM_EXTENSION), "requires lsm1 sqlite extension"
)
class TestLSM1Extension(BaseTestCase):
    ...
    def test_index_performance(self):
        ...
        self.assertEqual(data, [("k6", "v6"), ("k7", "v7"), ("k8", "v8"), ("k9", "v9")])

        del KVS[KVS.key.between("k10", "k18")]
        self.assertEqual(
            sorted([row.key for row in KVS[:"k2"]]), ["k0", "k1", "k19", "k2"]
        )

        del KVS["k3.1":"k8.1"]
        self.assertEqual(
            [row.key for row in KVS[:]], ["k0", "k1", "k19", "k2", "k3", "k9"]
        )
...
class TestIntWhereChain(ModelTestCase):
    ...
    def test_int_where_chain(self):
        with self.database.atomic():
            ...
            CalendarDay.insert_many(
                [{"month": jan, "value": i + 1} for i in range(31)]
            ).execute()
            CalendarDay.insert_many(
                [{"month": feb, "value": i + 1} for i in range(28)]
            ).execute()
...
class TestCollatedFieldDefinitions(ModelTestCase):
    ...
    def test_collated_fields(self):
        ...
        assertC(base.order_by(Datum.c.collate("NOCASE"), Datum.id), [2, 4, 3, 1])
