...
class TestDatabase(DatabaseTestCase):
    ...
    def test_context_settings(self):
        ...
        test_db2 = TestDatabase(
            None, field_types={"BIGINT": "XXX_BIGINT", "INT": "XXX_INT"}
        )
    ...
    def test_execute_sql(self):
        ...
        self.database.execute_sql(
            "INSERT INTO register (val) VALUES (?), (?)", (1337, 31337)
        )
...
class TestIntrospection(ModelTestCase):
    ...
    def test_get_indexes(self):
        ...
        data = [
            (index.name, index.columns, index.unique, index.table)
        ...
        self.assertEqual(
            sorted(data),
            [
                ("indexed_model_first_last", ["first", "last"], False, "indexed_model"),
                (
                    "indexed_model_first_last_dob",
                    ["first", "last", "dob"],
                    True,
                    "indexed_model",
                ),
    ...
    def test_get_columns(self):
        ...
        self.assertEqual(
            data,
            [("name", False, True, "category"), ("parent_id", True, False, "category")],
        )
    ...
    def test_get_views(self):
        def normalize_view_meta(view_meta):
            ...
            return (
                view_meta.name,
                (
                    sql_ws_norm.replace("`peewee_test`.", "")
                    .replace("`notes`.", "")
                    .replace("`", "")
                ),
            )
    ...
    def test_get_foreign_keys(self):
        ...
        data = [
            (fk.column, fk.dest_table, fk.dest_column, fk.table) for fk in foreign_keys
        ]
...
class TestAttachDatabase(ModelTestCase):
    ...
    def test_attach(self):
        ...
        (
            CacheData.insert_from(
                Data.select(), fields=[Data.id, Data.key, Data.value]
            ).execute()
        )
    ...
    def test_attach_detach(self):
        ...
        self.assertRaises(
            OperationalError, database.execute_sql, "select * from cache.sqlite_master"
        )
