...
class TestPwiz(BasePwizTestCase):
    ...
    def test_print_header(self):
        ...
        self.assertEqual(
            ...
            % (cmdline, db.database, peewee_version),
        )
...
class TestPwizUnknownField(BasePwizTestCase):
    ...
    def test_unknown_field(self):
        ...
        self.assertEqual(
            output.data.strip(),
            (
                self.header + self.unknown + self.basemodel + "class Foo(BaseModel):\n"
    ...
    def test_ignore_unknown(self):
        ...
        self.assertEqual(
            output.data.strip(),
            (
                self.header + self.basemodel + "class Foo(BaseModel):\n"
                "    unk1 = BareField(null=True)\n"
                "    # unk2 - BIZBAZ\n\n"
                "    class Meta:\n        table_name = 'foo'"
            ),
...
class TestPwizIntrospectViews(BasePwizTestCase):
    ...
    def setUp(self):
        ...
        self.database.execute_sql(
            'CREATE VIEW "events_public" AS ' "SELECT data FROM event WHERE status = 1"
        )
