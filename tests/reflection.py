...
class TestReflection(BaseReflectionTestCase):
    requires = [ColTypes, Nullable, RelModel, FKPK, Underscores, Category, Nugget]
    ...
    def test_generate_models(self):
        ...
        self.assertEqual(sorted(category._meta.fields), ["id", "name", "parent"])
        ...
        self.assertEqual(
            sorted(relmodel._meta.fields), ["col_types", "col_types_nullable", "id"]
        )
        ...
        self.assertEqual(relmodel.col_types.rel_model, models["col_types"])
        self.assertEqual(relmodel.col_types_nullable.rel_model, models["col_types"])
    ...
    def test_generate_models_indexes(self):
        ...
        self.assertEqual(
            sorted(indexes), [(["f10", "f11"], True), (["f11", "f8", "f13"], False),]
        )

    def test_table_subset(self):
        models = self.introspector.generate_models(
            table_names=["category", "col_types", "foobarbaz"]
        )
    ...
    def test_sqlite_fk_re(self):
        ...
        for test in user_id_tests:
            ...
            self.assertEqual(match.groups(), ("user_id", "users", "id",))

        for test in fk_pk_tests:
            match = re.search(regex, test, re.I)
            self.assertEqual(match.groups(), ("col_types_id", "coltypes", "f11",))
    ...
    def test_make_column_name(self):
        ...
        for col_name, expected in tests:
            self.assertEqual(self.introspector.make_column_name(col_name), expected)
    ...
    def test_make_model_name(self):
        ...
        for table_name, expected in tests:
            self.assertEqual(self.introspector.make_model_name(table_name), expected)
    ...
    def test_col_types(self):
        ...
        expected = (
            ...
            (
                "nullable",
                (("nullable_cf", CharField, True), ("nullable_if", IntegerField, True)),
            ),
            ("fkpk", (("col_types_id", ForeignKeyField, False),)),
            ("underscores", (("_id", AutoField, False), ("_name", CharField, False))),
            (
                "category",
                (("name", CharField, False), ("parent_id", ForeignKeyField, True)),
            ),
        )
    ...
    def test_column_meta(self):
        ...
        self.assertEqual(
            col_types_id.get_field_parameters(),
            {"column_name": "'col_types_id'", "model": "ColTypes", "field": "'f11'",},
        )

        col_types_nullable_id = rel_model["col_types_nullable_id"]
        self.assertEqual(
            col_types_nullable_id.get_field_parameters(),
            {
        ...
        fkpk = columns["fkpk"]
        self.assertEqual(
            fkpk["col_types_id"].get_field_parameters(),
            {
        ...
        self.assertEqual(
            parent_id.get_field_parameters(),
            {
        ...
        self.assertEqual(
            category_fk.get_field_parameters(),
            {"field": "'id'", "model": "Category", "column_name": "'category_id'",},
        )
    ...
    def test_get_field(self):
        ...
        expected = (
            ...
            (
                "nullable",
                (
                    ("nullable_cf", "nullable_cf = " "CharField(null=True)"),
            ...
            (
                "underscores",
                (("_id", "_id = AutoField()"), ("_name", "_name = CharField()"),),
            ),
            (
                "category",
                (
        ...
        for table, field_data in expected:
            for field_name, fields in field_data:
                ...
                self.assertTrue(actual in fields, "%s not in %s" % (actual, fields))
...
class TestReflectionDependencies(BaseReflectionTestCase):
    ...
    def test_generate_dependencies(self):
        ...
        self.assertEqual(
            set(ITweet._meta.fields), set(("id", "user", "content", "timestamp"))
        )
...
class TestReflectViews(BaseReflectionTestCase):
    ...
    def test_introspect_view(self):
        ...
        self.assertEqual(sorted(NotesPublic._meta.fields), ["content", "timestamp"])
    ...
    def test_introspect_view_integration(self):
        for i, (ct, st) in enumerate([("n1", 1), ("n2", 2), ("n3", 1)]):
            Note.create(
                content=ct, status=st, timestamp=datetime.datetime(2018, 1, 1 + i)
            )

        NP = self.introspector.generate_models(
            table_names=["notes_public"], include_views=True
        )["notes_public"]
        self.assertEqual(
            [(np.content, np.timestamp) for np in NP.select()],
            [
...
class TestInteractiveHelpers(ModelTestCase):
    ...
    def test_generate_models(self):
        ...
        assertFields(
            M["category"], [("id", "AUTO"), ("name", "VARCHAR"), ("parent", "INT")]
        )
        assertFields(
            M["event"],
            [
