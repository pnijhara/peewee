...
class TestModelAPIs(ModelTestCase):
    ...
    def test_bulk_create(self):
        ...
        if IS_POSTGRESQL:
            self.assertEqual(
                [u.id for u in User.select().order_by(User.id)],
    ...
    def test_bulk_create_batching(self):
        ...
        self.assertEqual(
            [u.username for u in User.select().order_by(User.id)], list("0123456789")
        )
    ...
    def test_bulk_create_composite_key(self):
        ...
        items = [CPK(key="k1", value=1, extra=1), CPK(key="k2", value=2, extra=2)]
        CPK.bulk_create(items)
        self.assertEqual(
            [(c.key, c.value, c.extra) for c in items], [("k1", 1, 1), ("k2", 2, 2)]
        )
    ...
    def test_bulk_update(self):
        data = [("f%s" % i, "l%s" % i, datetime.date(1980, i, i)) for i in range(1, 5)]
        ...
        self.assertEqual(
            [(p.first, p.last) for p in query],
            [("f1-x", "l1-x"), ("f2-y", "l2"), ("f3", "l3-z"), ("f4", "l4")],
        )
        ...
        self.assertEqual(
            [(p.first, p.last) for p in query],
            [("f1-x2", "l1-x"), ("f2-y2", "l2"), ("f3", "l3-z"), ("f4", "l4")],
        )
    ...
    def test_bulk_update_foreign_key(self):
        ...
        query = (
            Tweet.select(Tweet.content, User.username)
            .join(User)
            .order_by(Tweet.id)
            .objects()
        )
    ...
    def test_bulk_update_integrityerror(self):
        people = [
            Person(first="f%s" % i, last="l%s" % i, dob="1980-01-01") for i in range(10)
        ]
        Person.bulk_create(people)
    ...
    def test_bulk_update_cte(self):
        ...
        subq = cte.select(cte.c.x).where(CPK._meta.primary_key == (cte.c.k, cte.c.v))
    ...
    def test_insert_rowcount(self):
        ...
        query = User.select(User.username.concat("-x")).where(
            User.username.in_(["u1", "u2"])
        )
        ...
        query = User.select(User.username.concat("-y")).where(
            User.username.in_(["u3", "u4"])
        )
    ...
    def test_replace_rowcount(self):
        ...
        data = [("beanie", "cat", "999"), ("mickey", "dog", "123")]
        ...
        self.assertEqual(
            list(query.tuples()), [("beanie", "cat", "999"), ("mickey", "dog", "123")]
        )
    ...
    def test_get_shortcut(self):
        ...
        tweet = Tweet.get(content__ilike="w%", user__username__ilike="%ck%")
        self.assertEqual(tweet.content, "woof")

    @requires_models(User)
    def test_get_with_alias(self):
        huey = self.add_user("huey")
        query = User.select(User.username.alias("name")).where(User.username == "huey")
        obj = query.dicts().get()
    ...
    def test_get_or_none(self):
        huey = self.add_user("huey")
        self.assertEqual(User.get_or_none(User.username == "huey").username, "huey")
    ...
    def test_save_force(self):
        ...
        with self.database.atomic():
            self.assertRaises(IntegrityError, color.save, force_insert=True)
    ...
    def test_model_select(self):
        ...
        with self.assertQueryCount(1):
            ...
            tweets = list(query)
            self.assertEqual(
                [(t.content, t.user.username) for t in tweets],
                [
                    ("hiss", "huey"),
                    ("meow", "huey"),
                    ("purr", "huey"),
    ...
    def test_join_two_fks(self):
        ...
        with self.assertQueryCount(1):
            UA = User.alias()
            query = (
                Favorite.select(Favorite, Tweet, User, UA)
                .join(Tweet)
                .join(User)
                .switch(Favorite)
            ...
            accum = [
                (f.tweet.user.username, f.tweet.content, f.user.username) for f in query
            ]
    ...
    def test_join_empty_intermediate_model(self):
        ...
        self.assertEqual(accum, [("c111", "a1"), ("c112", "a1"), ("c211", "a2")])
        ...
        self.assertEqual(
            accum, [("c111", "b11", "a1"), ("c112", "b11", "a1"), ("c211", "b21", "a2")]
        )
    ...
    def test_join_empty_relations(self):
        ...
        with self.assertQueryCount(1):
            ...
            r = [(e.name, e.venue and e.venue.city.name or None) for e in query]
            self.assertEqual(
                r,
                [
                    ("House Party", "Topeka"),
    ...
    def test_join_same_model_twice(self):
        ...
        with self.assertQueryCount(1):
            query = (
                Relationship.select(Relationship, Person, PA)
        ...
        self.assertEqual(
            results, [("huey", "zaizee"), ("zaizee", "huey"), ("mickey", "huey")]
        )
    ...
    def test_peek(self):
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual(
                query.peek(n=2), [{"username": "huey"}, {"username": "mickey"}]
            )
    ...
    def test_multi_join(self):
        ...
        favorites = ((u1, t2_1), (u1, t2_2), (u2, t1_1), (u3, t1_2), (u3, t2_2))
        ...
        with self.assertQueryCount(1):
            query = (
                Favorite.select(
                    Favorite.id, Tweet.content, User.username, TweetUser.username
                )
                .join(Tweet)
    ...
    def test_join_subquery(self):
        ...
        with self.assertQueryCount(1):
            TA = Tweet.alias()
            max_q = (
                TA.select(TA.user, fn.MAX(TA.timestamp).alias("max_ts"))
                .group_by(TA.user)
                .alias("max_q")
            )
            ...
            latest = (
                Tweet.select(Tweet.user, Tweet.content, Tweet.timestamp)
                .join(max_q, on=predicate)
                .alias("latest")
            )

            query = User.select(User, latest.c.content, latest.c.timestamp).join(
                latest, on=(User.id == latest.c.user_id)
            )
        ...
        if not IS_SQLITE_OLD:
            self.assertEqual(data, [("huey", "hiss"), ("mickey", "grr")])

        with self.assertQueryCount(1):
            query = (
                Tweet.select(Tweet, User)
                .join(max_q, on=predicate)
                .switch(Tweet)
                .join(User)
            )
    ...
    def test_join_subquery_2(self):
        ...
        with self.assertQueryCount(1):
            users = User.select(User.id, User.username).where(
                User.username.in_(["huey", "zaizee"])
            )
            query = (
                Tweet.select(
                    Tweet.content.alias("content"), users.c.username.alias("username")
                )
            ...
            self.assertEqual(
                results, [("meow", "huey"), ("purr", "huey"), ("hiss", "huey")]
            )
    ...
    def test_join_subquery_cte(self):
        ...
        with self.assertQueryCount(1):
            ...
            query = (
                Tweet.select(Tweet.content, cte.c.username)
            ...
            self.assertEqual([t.content for t in query], ["meow", "purr", "hiss"])
    ...
    def test_subquery_emulate_window(self):
        ...
        # username.
        name2count = {"beanie": 6, "huey": 5, "mickey": 3, "pipey": 1, "zaizee": 4}
        names = []
        for name, count in sorted(name2count.items()):
            names += [name] * count
        User.insert_many(
            [(i, n) for i, n in enumerate(names, 1)], [User.id, User.username]
        ).execute()
        ...
        with self.assertQueryCount(1):
            ...
            UA = User.alias()
            query = (
                User.select(User.username, UA.id)
                .join(UA, on=((UA.username == User.username) & (UA.id >= User.id)))
    ...
    def test_subquery_alias_selection(self):
        ...
        with self.assertQueryCount(1):
            ...
            query = User.select(User.username, subq.alias("tweet_count")).order_by(
                User.id
            )
            self.assertEqual(
                [(u.username, u.tweet_count) for u in query],
                [("huey", 3), ("mickey", 2), ("zaizee", 0)],
            )
    ...
    def test_multi_update(self):
        ...
        self.assertEqual(
            [(u.id, u.username) for u in query], [(1, "u1x"), (2, "u2x"), (3, "u3")]
        )
    ...
    def test_compound_select(self):
        ...
        self.assertSQL(
            ...
            (
                ...
                'WHERE ("t2"."value" > ?) ORDER BY 2'
            ),
            [2, 7],
        )

        self.assertEqual(
            [row.value for row in c1], [0, 1, 8, 9], [row.__data__ for row in c1]
        )
    ...
    def test_union_column_resolution(self):
        ...
        self.assertEqual(sorted(results), [(1, "u1"), (2, "u2")])
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual(
                sorted(results),
                [(1, "u1-t1", "u1"), (2, "u1-t2", "u1"), (3, "u2-t1", "u2")],
            )
    ...
    def test_compound_select_as_subquery(self):
        ...
        q1 = (
            Tweet.select(Tweet.id, Tweet.content, User.username)
            .join(User)
        ...
        q = (
            union.select_from(union.c.username, fn.COUNT(union.c.id).alias("ct"))
    ...
    def test_union_with_join(self):
        ...
        with self.assertQueryCount(1):
            q1 = User.select(User, Tweet).join(
                Tweet, on=(Tweet.user == User.id).alias("foo")
            )
            q2 = User.select(User, Tweet).join(
                Tweet, on=(Tweet.user == User.id).alias("foo")
            )
    ...
    def test_union_cte(self):
        with self.database.atomic():
            (User.insert_many({"username": "u%s" % i} for i in range(10)).execute())
    ...
    def test_self_referential_fk(self):
        ...
        with self.assertQueryCount(1):
            ...
            query = (
                Category.select(Parent.name, Category.name)
    ...
    def test_empty_joined_instance(self):
        ...
        with self.assertQueryCount(1):
            Parent = Category.alias("p")
            query = (
                Category.select(Category, Parent)
                .join(Parent, JOIN.LEFT_OUTER, on=(Category.parent == Parent.name))
                .order_by(Category.name)
            )
            result = [(category.name, category.parent is None) for category in query]
    ...
    def test_from_multi_table(self):
        ...
        with self.assertQueryCount(1):
            query = (
                Tweet.select(Tweet, User)
                .from_(Tweet, User)
                .where((Tweet.user == User.id) & (User.username == "huey"))
                .order_by(Tweet.id)
                .dicts()
            )

            self.assertEqual([t["content"] for t in query], ["meow", "hiss", "purr"])
            self.assertEqual([t["username"] for t in query], ["huey", "huey", "huey"])
    ...
    def test_subquery_in_select_expression(self):
        ...
        with self.assertQueryCount(1):
            ...
            query = Point.select(Point.x, Point.y, subq.alias("sy")).order_by(
                Point.x, Point.y
            )
            self.assertEqual(
                list(query.tuples()), [(1, 1, 3), (1, 2, 3), (10, 10, 30), (10, 20, 30)]
            )
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual(list(query.tuples()), [(1, 4), (1, 5), (10, 40), (10, 50)])
    ...
    def test_filtering(self):
        ...
        with self.assertQueryCount(1):
            ...
            self.assertEqual([row.content for row in query], ["hiss", "meow", "purr"])

        with self.assertQueryCount(1):
            query = User.filter(tweets__content__ilike="w%")
            self.assertEqual([user.username for user in query], ["mickey", "mickey"])
    ...
    def test_table_schema(self):
        ...
        self.assertSQL(Schema.select(), ('SELECT "t1"."id" FROM "schema" AS "t1"'), [])

        Schema._meta.schema = "test"
        self.assertSQL(
            Schema.select(), ('SELECT "t1"."id" FROM "test"."schema" AS "t1"'), []
        )

        Schema._meta.schema = "another"
        self.assertSQL(
            Schema.select(), ('SELECT "t1"."id" FROM "another"."schema" AS "t1"'), []
        )
...
class TestRaw(ModelTestCase):
    ...
    def test_raw(self):
        ...
        query = User.raw(
            "SELECT username, SUBSTR(username, 1, 1) AS first "
            "FROM users "
        ...
        self.assertEqual(
            [(row.username, row.first) for row in query],
            [("chuck", "c"), ("charlie", "c")],
        )

    def test_raw_iterator(self):
        (User.insert_many([("charlie",), ("huey",)], fields=[User.username]).execute())
...
class TestDeleteInstance(ModelTestCase):
    ...
    def setUp(self):
        ...
        with self.database.atomic():
            ...
            Favorite.create(user=mickey, tweet=Tweet.create(user=huey, content="hiss"))
    ...
    def test_delete_instance_recursive(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                ('DELETE FROM "favorite" WHERE ("favorite"."user_id" = ?)', [huey.id]),
    ...
    def test_delete_nullable(self):
        ...
        self.assertEqual(
            sorted(queries),
            [
                ('DELETE FROM "account" WHERE ("account"."user_id" = ?)', [huey.id]),
                (
                    'DELETE FROM "favorite" WHERE ('
                    '"favorite"."tweet_id" IN ('
                    'SELECT "t1"."id" FROM "tweet" AS "t1" WHERE ('
...
class TestDefaultDirtyBehavior(ModelTestCase):
    ...
    def test_default_dirty(self):
        ...
        ac_db = AutoCounter.get((AutoCounter.counter == 1) & (AutoCounter.control == 1))
...
class TestDefaultValues(ModelTestCase):
    ...
    def test_default_present_on_insert(self):
        ...
        query = Sample.insert_many(
            [{"counter": "0"}, {"counter": 1, "value": 2}, {"counter": "2"}]
        )
        self.assertSQL(
            query,
            (
        ...
        query = Sample.insert_many([(0,), (1, 2.0)], fields=[Sample.counter])
        self.assertSQL(
            query,
            ('INSERT INTO "sample" ("counter", "value") ' "VALUES (?, ?), (?, ?)"),
            [0, 1.0, 1, 2.0],
    ...
    def test_default_present_on_create(self):
        ...
        self.assertEqual(s_db.value, 1.0)

    def test_defaults_from_cursor(self):
        s = Sample.create(counter=1)
        sm1 = SampleMeta.create(sample=s, value=1.0)
        sm2 = SampleMeta.create(sample=s, value=2.0)
        ...
        with self.assertQueryCount(1):
            # Join-graph query.
            query = (
                SampleMeta.select(SampleMeta.sample, Sample.counter)
                .join(Sample)
...
class TestFunctionCoerce(ModelTestCase):
    ...
    def test_conv_using_python_value(self):
        ...
        counter = fn.GROUP_CONCAT(Sample.counter).python_value(
...
class TestJoinModelAlias(ModelTestCase):
    data = (("huey", "meow"), ("huey", "purr"), ("zaizee", "hiss"), ("mickey", "woof"))
    requires = [User, Tweet]
    ...
    def _test_query(self, alias_expr):
        UA = alias_expr()
        return Tweet.select(Tweet, UA).order_by(UA.username, Tweet.content)

    def assertTweets(self, query, user_attr="user"):
        with self.assertQueryCount(1):
            data = [
                (getattr(tweet, user_attr).username, tweet.content) for tweet in query
            ]
    ...
    def test_join_aliased_columns(self):
        query = Tweet.select(Tweet.id.alias("tweet_id"), Tweet.content).order_by(
            Tweet.id
        )
        self.assertEqual(
            [(t.tweet_id, t.content) for t in query],
            [(1, "meow"), (2, "purr"), (3, "hiss"), (4, "woof")],
        )

        query = (
            Tweet.select(Tweet.id.alias("tweet_id"), Tweet.content)
            .join(User)
            .where(User.username == "huey")
            .order_by(Tweet.id)
        )
        self.assertEqual(
            [(t.tweet_id, t.content) for t in query], [(1, "meow"), (2, "purr")]
        )
    ...
    def _test_query_backref(self, alias_expr):
        TA = alias_expr()
        return User.select(User, TA).order_by(User.username, TA.content)

    def assertUsers(self, query, tweet_attr="tweet"):
        with self.assertQueryCount(1):
            data = [
                (user.username, getattr(user, tweet_attr).content) for user in query
            ]
        self.assertEqual(sorted(self.data), data)
    ...
    def test_join_alias_twice(self):
        ...
        with self.assertQueryCount(1):
            ...
            data = [
                (row.username, row.tweet.content, row.tweet.foo.username)
                for row in query
            ]
    ...
    def test_alias_filter(self):
        ...
        for lookup in lookups:
            with self.assertQueryCount(1):
                query = (
                    Tweet.select(Tweet.content, UA.username)
                    .join(UA)
                    .filter(**lookup)
...
class TestWindowFunctionIntegration(ModelTestCase):
    ...
    def test_simple_partition(self):
        query = (
            Sample.select(
                Sample.counter,
                Sample.value,
                fn.AVG(Sample.value).over(partition_by=[Sample.counter]),
            )
        ...
        query = (
            Sample.select(
                Sample.counter, Sample.value, fn.AVG(Sample.value).over(window)
            )
            .window(window)
            .order_by(Sample.counter, Sample.value)
            .tuples()
    ...
    def test_mixed_ordering(self):
        ...
        query = Sample.select(Sample.counter, Sample.value, s.alias("rtotal")).order_by(
            Sample.id
        )
        ...
        # 3 | 100 | (1 + 3 + 10 + 20 + 100)
        self.assertEqual(
            [(r.counter, r.value, r.rtotal) for r in query],
            [
                (1, 10.0, 14.0),
                (1, 20.0, 34.0),
                (2, 1.0, 1.0),
    ...
    def test_reuse_window(self):
        ...
        results = [
            (r.counter, r.value, r.quartile, r.quintile, r.percentile) for r in query
        ]
        self.assertEqual(
            results,
            [
    ...
    def test_ordered_window(self):
        window = Window(partition_by=[Sample.counter], order_by=[Sample.value.desc()])
        query = (
            Sample.select(
                Sample.counter,
                Sample.value,
    ...
    def test_empty_over(self):
        ...
        self.assertEqual(
            list(query),
            [(1, 10.0, None), (1, 20.0, 1), (2, 1.0, 1), (2, 3.0, 2), (3, 100.0, 2)],
        )
    ...
    def test_bounds(self):
        ...
        self.assertEqual(
            list(query),
            [(10.0, 30.0), (20.0, 30.0), (1.0, 4.0), (3.0, 4.0), (100.0, 100.0)],
        )
        ...
        query = (
            Sample.select(
                ...
                Sample.value,
                fn.SUM(Sample.value).over(
                    order_by=[Sample.id], start=Window.preceding(2)
                ),
            )
            .order_by(Sample.id)
            .tuples()
    ...
    def test_frame_types(self):
        Sample.create(counter=1, value=20.0)
        Sample.create(counter=2, value=1.0)  # Observe logical peer handling.
        ...
        query = Sample.select(
            ...
            Sample.value,
            fn.SUM(Sample.value).over(
                order_by=[Sample.counter, Sample.value], frame_type=Window.ROWS
            ),
        ).order_by(Sample.counter, Sample.value)
        self.assertEqual(
            list(query.tuples()),
        ...
        query = Sample.select(
            ...
            Sample.value,
            fn.SUM(Sample.value).over(
                order_by=[Sample.counter, Sample.value], start=Window.preceding(2)
            ),
        ).order_by(Sample.counter, Sample.value)
        self.assertEqual(
            list(query.tuples()),
    ...
    def test_filter_clause(self):
        condsum = (
            ...
            .over(order_by=[Sample.id], start=Window.preceding(1))
        )
        query = Sample.select(
            Sample.counter, Sample.value, condsum.alias("cs")
    ...
    @skip_if(
        IS_MYSQL or (IS_SQLITE and not IS_SQLITE_30), "requires FILTER with aggregates"
    )
    def test_filter_with_aggregate(self):
        condsum = fn.SUM(Sample.value).filter(Sample.counter > 1)
        query = (
            Sample.select(Sample.counter, condsum.alias("cs"))
            .group_by(Sample.counter)
            .order_by(Sample.counter)
        )
        self.assertEqual(list(query.tuples()), [(1, None), (2, 4.0), (3, 100.0)])
...
class TestForUpdateIntegration(ModelTestCase):
    ...
    def test_for_update(self):
        ...
        with self.database.manual_commit():
            ...
            updated = (
                User.update(username="ziggy").where(User.username == "zaizee").execute()
            )
            self.assertEqual(updated, 1)

            if IS_POSTGRESQL:
                nrows = (
                    AltUser.update(username="huey-x")
                    .where(AltUser.username == "huey")
                    .execute()
                )
                self.assertEqual(nrows, 1)

            query = AltUser.select(AltUser.username).where(AltUser.id == zaizee.id)
    ...
    def test_for_update_nested(self):
        ...
        nrows = User.delete().where(User.id.in_(subq)).execute()
    ...
    def test_for_update_of(self):
        ...
        with self.database.manual_commit():
            ...
            nrows = AltTweet.update(content="zx").where(AltTweet.user == z.id).execute()
            self.assertEqual(nrows, 1)
        ...
        self.assertEqual(
            [(t.content, t.user.username) for t in query],
            [("h", "huey-x"), ("zx", "zaizee")],
        )
...
class TestReturningIntegration(ModelTestCase):
    ...
    def test_simple_returning(self):
        ...
        self.assertSQL(
            query,
            ('INSERT INTO "users" ("username") VALUES (?) ' 'RETURNING "users"."id"'),
            ["charlie"],
        )
        ...
        query = User.insert(username="zaizee").returning(User.id, User.username).dicts()
        self.assertSQL(
            query,
            (
        ...
        query = User.insert(username="mickey").returning(User).objects()
        self.assertSQL(
            query,
            (
                'INSERT INTO "users" ("username") VALUES (?) '
        ...
        # Can specify aliases.
        query = User.insert(username="sipp").returning(
            User.username.alias("new_username")
        )
    ...
    def test_simple_returning_insert_update_delete(self):
        ...
        res = User.delete().where(User.id == 1).returning(User).execute()
        # Subsequent iterations are cached.
        for _ in range(2):
            self.assertEqual([u.username for u in res], ["charlie2"])
    ...
    def test_returning_server_defaults(self):
        query = ServerDefault.insert().returning(
            ServerDefault.id, ServerDefault.timestamp
        )
        self.assertSQL(
            query,
            (
    ...
    def test_returning_multi(self):
        ...
        data = query.execute()
        self.assertEqual(
            [(row.id, row.username) for row in data],
            [(3, "foo"), (4, "bar"), (5, "baz")],
        )
    ...
    def test_update_returning(self):
        id_list = User.insert_many(
            [{"username": "huey"}, {"username": "zaizee"}]
        ).execute()
        huey_id, zaizee_id = [pk for pk, in id_list]

        query = (
            User.update(username="ziggy")
            .where(User.username == "zaizee")
            .returning(User.id, User.username)
        )
        self.assertSQL(
...
class TestCTEIntegration(ModelTestCase):
    ...
    @skip_if(IS_SQLITE_OLD or (IS_MYSQL and not IS_MYSQL_ADVANCED_FEATURES) or IS_CRDB)
    ...
    def test_docs_example(self):
        ...
        base = (
            Member.select(Member.recommendedby)
            .where(Member.id == gen3_3_1.id)
            .cte("recommenders", recursive=True, columns=("recommender",))
        )

        MA = Member.alias()
        recursive = MA.select(MA.recommendedby).join(
            base, on=(MA.id == base.c.recommender)
        )

        cte = base.union_all(recursive)
        query = (
            cte.select_from(cte.c.recommender, Member.name)
    ...
    def test_simple_cte(self):
        cte = Category.select(Category.name, Category.parent).cte(
            "catz", columns=("name", "parent")
        )
        ...
        query = (
            Category.select(cte.c.name, cte.c.parent.alias("pname"))
            .from_(cte)
            .order_by(cte.c.name)
            .with_cte(cte)
        )
        ...
        query2 = cte.select_from(cte.c.name, cte.c.parent.alias("pname")).order_by(
            cte.c.name
        )
        self.assertSQL(query2, cte_sql, [])

        self.assertEqual(
            [(row.name, row.pname) for row in query],
            [
                ("c11", "p1"),
                ("c12", "p1"),
                ("c31", "p3"),
    ...
    def test_cte_join(self):
        cte = Category.select(Category.name).cte("parents", columns=("name",))

        query = (
            Category.select(Category.name, cte.c.name.alias("pname"))
        ...
        self.assertEqual(
            [(c.name, c.parents["pname"]) for c in query],
            [
                ("c11", "p1"),
                ("c12", "p1"),
                ("c31", "p3"),
    ...
    def test_recursive_cte(self):
        def get_parents(cname):
            ...
            base = (
                C1.select(C1.name, C1.parent, level, path)
                .where(C1.name == cname)
            ...
            recursive = (
                C2.select(C2.name, C2.parent, rlevel, rpath)
                .from_(base)
                .join(C2, on=(C2.name == base.c.parent_id))
            )

            cte = base + recursive
            query = cte.select_from(cte.c.name, cte.c.level, cte.c.path).order_by(
                cte.c.level
            )
            self.assertSQL(
                query,
        ...
        self.assertEqual(
            data,
            [("c31", 1, "c31"), ("p3", 2, "c31->p3"), ("root", 3, "c31->p3->root")],
        )

        data = [(c.name, c.level, c.path) for c in get_parents("c12").namedtuples()]
        self.assertEqual(
            data,
            [("c12", 1, "c12"), ("p1", 2, "c12->p1"), ("root", 3, "c12->p1->root")],
        )
    ...
    def test_recursive_cte2(self):
        hierarchy = (
            Category.select(Category.name, Value(0).alias("level"))
            .where(Category.parent.is_null(True))
            .cte(name="hierarchy", recursive=True)
        )

        C = Category.alias()
        recursive = C.select(C.name, (hierarchy.c.level + 1).alias("level")).join(
            hierarchy, on=(C.parent == hierarchy.c.name)
        )

        cte = hierarchy.union_all(recursive)
        query = cte.select_from(cte.c.name, cte.c.level).order_by(cte.c.name)
        self.assertEqual(
            [(r.name, r.level) for r in query],
            [
                ("c11", 2),
                ("c12", 2),
                ("c31", 2),
    ...
    def test_recursive_cte_docs_example(self):
        ...
        base_case = (
            Base.select(Base.name, Base.parent, level, path)
        ...
        recursive = RTerm.select(RTerm.name, RTerm.parent, rlevel, rpath).join(
            base_case, on=(RTerm.parent == base_case.c.name)
        )
    ...
    def test_cte_reuse_aggregate(self):
        data = ((1, (1.25, 1.5, 1.75)), (2, (2.1, 2.3, 2.5, 2.7, 2.9)), (3, (3.5, 3.5)))
        with self.database.atomic():
            for counter, values in data:
                (
                    Sample.insert_many(
                        [(counter, value) for value in values],
                        fields=[Sample.counter, Sample.value],
                    ).execute()
                )

        cte = (
            Sample.select(Sample.counter, fn.AVG(Sample.value).alias("avg_value"))
            .group_by(Sample.counter)
        ...
        query = (
            Sample.select(
                Sample.counter, (Sample.value - cte.c.avg_value).alias("diff")
            )
        ...
        self.assertEqual(
            [(a, round(b, 2)) for a, b in query.tuples()],
            [(1, 0.25), (2, 0.2), (2, 0.4)],
        )
...
class TestTupleComparison(ModelTestCase):
    ...
    def test_tuples(self):
        ...
        query = User.select().where(Tuple(User.username, User.id) == ("b", ub.id))
        self.assertSQL(
            query,
            (
                'SELECT "t1"."id", "t1"."username" FROM "users" AS "t1" '
                'WHERE (("t1"."username", "t1"."id") = (?, ?))'
            ),
            ["b", ub.id],
        )
    ...
    def test_tuple_subquery(self):
        ua, ub, uc = [User.create(username=username) for username in "abc"]
        UA = User.alias()
        subquery = UA.select(UA.username, UA.id).where(UA.username != "b")
...
class TestFieldInheritance(BaseTestCase):
    def test_field_inheritance(self):
        ...
        self.assertEqual(
            BasePost._meta.sorted_field_names, ["id", "content", "timestamp"]
        )
        self.assertEqual(
            BasePost._meta.sorted_fields,
            [BasePost.id, BasePost.content, BasePost.timestamp],
        )

        self.assertEqual(
            Photo._meta.sorted_field_names, ["id", "content", "timestamp", "image"]
        )
        self.assertEqual(
            Photo._meta.sorted_fields,
            [Photo.id, Photo.content, Photo.timestamp, Photo.image],
        )

        self.assertEqual(
            Note._meta.sorted_field_names, ["id", "content", "timestamp", "category"]
        )
        self.assertEqual(
            Note._meta.sorted_fields,
            [Note.id, Note.content, Note.timestamp, Note.category],
        )
    ...
    def test_foreign_key_field_inheritance(self):
        ...
        self.assertEqual(
            BasePost._meta.sorted_field_names, ["id", "category", "timestamp"]
        )
        self.assertEqual(
            BasePost._meta.sorted_fields,
            [BasePost.id, BasePost.category, BasePost.timestamp],
        )

        self.assertEqual(
            Photo._meta.sorted_field_names, ["id", "category", "timestamp", "image"]
        )
        self.assertEqual(
            Photo._meta.sorted_fields,
            [Photo.id, Photo.category, Photo.timestamp, Photo.image],
        )

        self.assertEqual(
            Note._meta.sorted_field_names, ["id", "category", "timestamp", "content"]
        )
        self.assertEqual(
            Note._meta.sorted_fields,
            [Note.id, Note.category, Note.timestamp, Note.content],
        )

        self.assertEqual(
            Category._meta.backrefs,
            {BasePost.category: BasePost, Photo.category: Photo, Note.category: Note},
        )
    ...
    def test_foreign_key_pk_inheritance(self):
        ...
        self.assertEqual(
            Account._meta.backrefs,
            {Admin.account: Admin, User.account: User, BaseUser.account: BaseUser},
        )
    ...
    def test_backref_inheritance(self):
        ...
        self.assertEqual(
            Category._meta.backrefs,
            {BasePost.category: BasePost, Note.category: Note, Photo.category: Photo},
        )
    ...
    def test_deferred_fk_creation(self):
        ...
        try:
            ...
            with db.atomic():
                self.assertRaises(IntegrityError, B._schema.create_foreign_key, B.a)
...
class OnConflictTests(object):
    ...
    def assertData(self, expected):
        query = Emp.select(Emp.first, Emp.last, Emp.empno).order_by(Emp.id).tuples()
        self.assertEqual(list(query), expected)

    def test_ignore(self):
        query = (
            Emp.insert(first="foo", last="bar", empno="123")
            .on_conflict("ignore")
            .execute()
        )
...
def requires_upsert(m):
    return skip_unless(IS_SQLITE_24 or IS_POSTGRESQL or IS_CRDB, "requires upsert")(m)
...
class PGOnConflictTests(OnConflictTests):
    ...
    def test_update(self):
        ...
        res = (
            Emp.insert(first="foo", last="bar", empno="125")
            .on_conflict(
        ...
        self.assertData(
            [("huey", "cat", "123"), ("zaizee", "cat", "124"), ("foo", "baze", "125.1")]
        )
    ...
    def test_update_atomic(self):
        ...
        query = OCTest.insert(a="foo", b=1).on_conflict(
            conflict_target=(OCTest.a,), update={OCTest.b: OCTest.b + 2}
        )
    ...
    def test_conflict_update_excluded(self):
        ...
        res = (
            Emp.insert(first="huey", last="c", empno="30")
            .on_conflict(
                conflict_target=(Emp.first, Emp.last),
    ...
    def test_conflict_update_excluded2(self):
        ...
        query = KV.insert(key="k1", value=10).on_conflict(
    ...
    def test_conflict_target_constraint_where(self):
        ...
        query = (
            UKVP.select(UKVP.key, UKVP.value, UKVP.extra)
            .order_by(UKVP.key, UKVP.value, UKVP.extra)
            .tuples()
        )

        self.assertEqual(list(query), [("k1", 1, 1), ("k1", 1, 2), ("k2", 2, 3)])
...
class TestUpsertMySQL(OnConflictTests, ModelTestCase):
    def test_replace(self):
        ...
        query = (
            Emp.insert(first="mickey", last="dog", empno="1337")
            .on_conflict("replace")
            .execute()
        )
        self.assertData(
        ...
        query = (
            Emp.insert(first="nuggie", last="dog", empno="123")
            .on_conflict("replace")
            .execute()
        )
        self.assertData(
        ...
        query = (
            Emp.insert(first="beanie", last="cat", empno="126")
            .on_conflict("replace")
            .execute()
        )
        self.assertData(
    ...
    @requires_models(OCTest)
    def test_update(self):
        pk = OCTest.insert(a="a", b=3).on_conflict(update={OCTest.b: 1337}).execute()
        oc = OCTest.get(OCTest.a == "a")
        self.assertEqual(oc.b, 3)

        pk2 = (
            OCTest.insert(a="a", b=4)
        ...
        pk3 = OCTest.insert(a="a2", b=5).on_conflict(update={OCTest.b: 1337}).execute()
    ...
    def test_update_preserve(self):
        ...
        pk = OCTest.insert(a="a", b=4).on_conflict(preserve=[OCTest.b]).execute()
        oc = OCTest.get(OCTest.a == "a")
        self.assertEqual(oc.b, 4)

        pk2 = (
            OCTest.insert(a="a", b=5, c=6)
            .on_conflict(preserve=[OCTest.c], update={OCTest.b: OCTest.b + 100})
            .execute()
        )
        self.assertEqual(pk, pk2)
        self.assertEqual(OCTest.select().count(), 1)
...
class TestUpsertSqlite(PGOnConflictTests, ModelTestCase):
    ...
    def test_do_nothing(self):
        query = Emp.insert(first="foo", last="bar", empno="123").on_conflict("nothing")
        self.assertSQL(
            query,
            (
...
class TestUpsertPostgresql(PGOnConflictTests, ModelTestCase):
    ...
    def test_conflict_target_constraint(self):
        ...
        ret = (
            UKV.insert(key="k1", value="v1", extra="e1")
            .on_conflict(conflict_target=(UKV.key, UKV.value), preserve=(UKV.extra,))
            .execute()
        )
        ...
        ret = (
            UKV.insert(key="k2", value="v2", extra="e2")
            .on_conflict(conflict_constraint="ukv_key_value", preserve=(UKV.extra,))
            .execute()
        )
        ...
        ret = (
            UKV.insert(key="k3", value="v3", extra="e3")
            .on_conflict(conflict_target=[UKV.key, UKV.value], preserve=[UKV.extra])
            .execute()
        )
    ...
    def test_conflict_ambiguous_column(self):
        ...
        query = UKVRel.insert_from(
            subq, [UKVRel.key, UKVRel.value, UKVRel.extra]
        ).on_conflict(
            conflict_target=[UKVRel.key, UKVRel.value],
        ...
        self.assertSQL(
            ...
            (
                ...
                'WHERE ("ukv_rel"."key" != ?) RETURNING "ukv_rel"."id"'
            ),
            ["k2"],
        )

        query.execute()
        query = UKVRel.select(UKVRel.key, UKVRel.value, UKVRel.extra).order_by(
            UKVRel.key
        )
        self.assertEqual(
            list(query.tuples()),
            [("k1", "v1", "e1"), ("k2", "v2", "x2"), ("k3", "v3", "e1")],
        )
...
class TestJoinSubquery(ModelTestCase):
    ...
    def test_join_subquery(self):
        ...
        def get_person(name):
            if name not in people:
                people[name] = Person.create(
                    first=name, last=name, dob=datetime.date(2017, 1, 1)
                )
            return people[name]
        ...
        subq = (
            Relationship.select(
                Friend.first.alias("friend_name"), Relationship.from_person
            )
        ...
        # single query.
        query = (
            Person.select(Person.first, subq.c.friend_name)
            .join(subq, JOIN.LEFT_OUTER, on=(Person.id == subq.c.from_person_id))
...
class TestSumCase(ModelTestCase):
    ...
    def test_sum_case(self):
        ...
        e_sum = fn.SUM(case)
        query = (
            User.select(User.username, e_sum.alias("e_sum"))
        ...
        self.assertEqual(data, [("charlie", 1), ("huey", 0), ("zaizee", 1)])
...
class TestMaxAlias(ModelTestCase):
    ...
    def test_max_alias(self):
        ...
        with self.assertQueryCount(1):
            ...
            query = (
                Transaction.select(amount, TUser.username)
                .join(TUser)
        ...
        self.assertEqual(data, [(30.0, "charlie"), (2.5, "huey")])
...
class TestCompoundSelectModels(ModelTestCase):
    ...
    def test_mix_models_with_model_row_type(self):
        ...
        lhs = CNote.select(
            CNote.id.cast(cast).alias("id_text"), CNote.content, CNote.timestamp
        )
    ...
    def test_mixed_models_tuple_row_type(self):
        cast = "CHAR" if IS_MYSQL else "TEXT"
        lhs = CNote.select(
            CNote.id.cast(cast).alias("id"), CNote.content, CNote.timestamp
        )
        rhs = CFile.select(CFile.filename, CFile.data, CFile.timestamp)
...
class TestUpdateFromIntegration(ModelTestCase):
    ...
    def test_update_from(self):
        ...
        (
            User.update({User.username: vl.c.username})
            .from_(vl)
    ...
    def test_update_from_subselect(self):
        ...
        (
            User.update({User.username: subq.c.username})
            .from_(subq)
    ...
    def test_update_from_simple(self):
        ...
        (
            User.update({User.username: Tweet.content})
            .from_(Tweet)
...
class TestLateralJoin(ModelTestCase):
    ...
    def test_lateral_join(self):
        ...
        # trying to find.
        user_query = User.select(User.id, User.username).order_by(User.id).alias("uq")
        ...
        join = NodeList(
            (user_query, SQL("LEFT JOIN LATERAL"), tweet_query, SQL("ON %s", [True]))
        )
        query = (
            Tweet.select(user_query.c.username, tweet_query.c.content)
            .from_(join)
            .dicts()
        )
        self.assertEqual(
...
class TestMultiSelfJoin(ModelTestCase):
    ...
    def test_multi_self_join(self):
        ...
        Heading = Task.alias()
        query = (
            Task.select(Task, Project, Heading)
            .join(
                Heading,
                JOIN.LEFT_OUTER,
...
class TestModelConstraints(ModelTestCase):
    ...
    def test_model_constraints(self):
        ...
        with self.database.atomic():
            self.assertRaises(
                IntegrityError, Product.create, name="p2", price=0, status="a"
            )
        ...
        with self.database.atomic():
            self.assertRaises(
                IntegrityError, Product.create, name="p3", price=1, status="x"
            )
...
class TestModelFieldReprs(BaseTestCase):
    def test_model_reprs(self):
        ...
        self.assertEqual(repr(EAV(entity="e", attribute="a")), "<EAV: ('e', 'a')>")
...
class TestManyToManyJoining(ModelTestCase):
    ...
    def test_join_subquery(self):
        courses = Course.select(Course.id, Course.name).order_by(Course.id).limit(5)
        query = (
            Attendance.select(Attendance, Student, courses.c.name)
            .join_from(Attendance, Student)
            .join_from(Attendance, courses, on=(Attendance.course == courses.c.id))
        )
    ...
    def test_join_where_subquery(self):
        ...
        query = (
            Attendance.select(Attendance, Student, Course)
            .join_from(Attendance, Student)
