...
class TestCockroachDatabase(ModelTestCase):
    ...
    def test_retry_transfer_example(self):
        ...
        def transfer_funds(from_k, to_k, amt):
            ...
            (from_v,) = (
                KV.update(v=KV.v - amt).where(KV.k == from_k).returning(KV.v).execute()
            )
            (to_v,) = (
                KV.update(v=KV.v + amt).where(KV.k == to_k).returning(KV.v).execute()
            )
    ...
    def test_uuid_key_as_fk(self):
        ...
        with self.assertQueryCount(1):
            query = (
                UIDNote.select(UIDNote, UID)
                .join(UID)
                .where(UID.title == "u2")
                .order_by(UIDNote.note)
            )
            self.assertEqual(
                [(un.note, un.uid.title) for un in query],
                [("u2-1", "u2"), ("u2-2", "u2")],
            )

        query = (
            UID.select(UID, fn.COUNT(UIDNote.id).alias("note_count"))
            .join(UIDNote, JOIN.LEFT_OUTER)
            .group_by(UID)
            .order_by(fn.COUNT(UIDNote.id).desc())
        )
        self.assertEqual(
            [(u.title, u.note_count) for u in query], [("u2", 2), ("u1", 1), ("u3", 0)]
        )
...
class TestCockroachIntegration(ModelTestCase):
    ...
    def test_compound_select(self):
        ...
        self.assertEqual(
            [(obj.k, obj.v) for obj in query], [(10, 1), (20, 2), (30, 3), (40, 4)]
        )
